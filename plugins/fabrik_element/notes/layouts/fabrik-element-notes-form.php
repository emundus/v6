...
if ($d->canUse) :
    ...
    if ($d->primaryKey > 0) :
        if ($d->fieldType == 'field') :?>
            <input class="fabrikinput inputbox text col-sm-12" name="<?php echo $d->name; ?>" />
        <?php
        else:
            ?>
            <textarea class="fabrikinput inputbox text col-sm-12" name="<?php echo $d->name; ?>" cols="50" rows="3" /></textarea>
        <?php
        endif;
        ?>
