 ...
 * @subpackage  Fabrik.element.emundusdatabasejoin
 ...
 * @since       3.0
 */
class PlgFabrik_ElementEmundusdatabasejoin extends PlgFabrik_ElementList
{
	/**
	 * connection
	 *
	...
	public function getAsField_html(&$aFields, &$aAsFields, $opts = array())
	{
		...
		if ($params->get('join_conn_id') == $connection->get('id') || $element->plugin != 'emundusdatabasejoin')
		{
			$join = $this->getJoin();
	...
	public function getJoinsToThisKey(&$table)
	{
		...
		$query->select('*, t.label AS tablelabel')->from('#__{package}_elements AS el')
			...
			->where('plugin = ' . $db->quote('emundusdatabasejoin'))
	...
	protected function _getOptionVals($data = array(), $repeatCounter = 0, $incWhere = true, $opts = array())
	{
		...
		$db->setQuery($sql);
		FabrikHelperHTML::debug((string) $db->getQuery(), $this->getElement()->name . ' emundusdatabasejoin element: get options query');
		$this->optionVals[$sqlKey] = $db->loadObjectList();
		FabrikHelperHTML::debug($this->optionVals, 'emundusdatabasejoin elements');
	...
	protected function renderFrontEndSelect($data, $repeatCounter = 0)
	{
		...
		$displayData->frontEndSelect = $params->get('fabrikemundusdatabasejoin_frontend_select');
		$displayData->frontEndAdd    = $params->get('fabrikemundusdatabasejoin_frontend_add');
		$forms                       = $this->getLinkedForms();
		$popupForm                   = (int) $params->get('emundusdatabasejoin_popupform');
	...
	protected function addReadOnlyLinks(&$defaultLabels, $defaultValues)
	{
		...
		if ($this->app->input->get('print', '') !== '1' && $params->get('emundusdatabasejoin_readonly_link') == 1)
		{
	...
	{
		$params      = $this->getParams();
		$popupFormId = (int) $params->get('emundusdatabasejoin_popupform');
	...
	protected function filterValueList_All($normal, $tableName = '', $label = '', $id = '', $incjoin = true)
	{
		...
		*/
		...
		FabrikHelperHTML::debug((string) $fabrikDb->getQuery(), 'fabrikemundusdatabasejoin getFilter');
	...
	public function elementJavascript($repeatCounter)
	{
		...
		if ($this->getParams()->get('database_join_display_type', 'dropdown') == 'auto-complete')
		{
			...
			FabrikHelperHTML::autoComplete($id, $this->getElement()->get('id'), $this->getFormModel()->getId(), 'emundusdatabasejoin', $autoOpts);
		}

		$opts = $this->elementJavascriptOpts($repeatCounter);

		return array('FbEmundusdatabasejoin', $id, $opts);
	}
	...
	public function jsJLayouts()
	{
		...
		if ($params->get('fabrikemundusdatabasejoin_frontend_select'))
		{
			$modalOpts = array(
				'content' => '',
				'id' => 'db_join_select',
	...
	protected function elementJavascriptOpts($repeatCounter = 0)
	{
		...
		$opts->autoCompleteOpts   = $opts->displayType == 'auto-complete'
			? FabrikHelperHTML::autoCompleteOptions($opts->id, $this->getElement()->get('id'), $this->getFormModel()->getId(), 'emundusdatabasejoin') : null;
		$opts->allowadd           = $params->get('fabrikemundusdatabasejoin_frontend_add', 0) == 0 ? false : true;
		...
		if ($params->get('emundusdatabasejoin_where_ajax', '0') === '1')
		{
	...
	public function onAjax_getOptions()
	{
		...
		$eval = $params->get('emundusdatabasejoin_where_ajax_default_eval');
	 ...
	 *
	 * @param   PlgFabrik_ElementEmundusdatabasejoin $elementModel element model
	...
	public function selfDiagnose()
	{
		...
		if ($this->pluginName == 'emundusdatabasejoin')
		{
			$params = $this->getParams();
	...
	/**
	 * Should the 'label' field be quoted.  Overridden by emundusdatabasejoin and extended classes,
