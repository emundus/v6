...
class plgFabrik_ElementEmundusreferent extends plgFabrik_Element
{
    ...
    public function onAjax_getOptions()
    {
        ...
        // check if this <referee,attachment,fnum> is selected
        $query->clear()
            ->select('count(*)')
            ->from($db->quoteName('#__emundus_files_request', 'jefr'))
            ->where($db->quoteName('jefr.fnum') . ' LIKE ' . $db->quote($fnum))
            ->andWhere($db->quoteName('jefr.email') . ' = ' . $db->quote($recipient))
        ...
        if ($isSelectedReferent == 0) {
            ...
            // if this letter is not uploaded by referee
            if (!$this->isRequestReferentLetterUploaded($attachment_id, $fnum, $fnum_detail['applicant_id'])) {
                // check if the last request >= 60 minutes --> 3600 seconds

                $query->clear()
                    ->select('count(*)')
                    ->from($db->quoteName('#__emundus_files_request'))
                    ->where($db->quoteName('student_id') . ' = ' . $db->quote($fnum_detail['applicant_id']))
                    ->andWhere($db->quoteName('attachment_id') . ' = ' . $db->quote($attachment_id))
                    ->andWhere($db->quoteName('fnum') . ' LIKE ' . $db->quote($fnum))
                    ->andWhere('time_to_sec(timediff(now(),(select time_date from jos_emundus_files_request as jefr where jefr.email' . ' = ' . $db->quote($recipient) . 'order by id desc limit 1))) < 3600');

                $db->setQuery($query);
                $validRequest = $db->loadResult();

                if ($validRequest == 0) {
                    $flag = true;       // the last request is more than 1 hour --> will sent
                }
            }

        }

        // send email if and only if $flag is true
        if ($flag) {
    ...
    // table "jos_emundus_files_request"
    protected function isRequestReferentLetterUploaded($attachment_id, $fnum, $student_id) {
        $db = JFactory::getDBO();
        $query = 'SELECT count(id) as cpt FROM #__emundus_files_request WHERE student_id=' . $student_id . ' AND attachment_id = ' . $attachment_id . ' AND uploaded = 1 AND fnum LIKE ' . $db->quote($fnum);
