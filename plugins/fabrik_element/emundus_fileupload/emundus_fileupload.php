...
class PlgFabrik_ElementEmundus_fileupload extends PlgFabrik_Element
{
    ...
    public function onBeforeStore()
    {
        ...
        // find the name of fileupload with the tablename and the fabrik element name
        $formid = $jinput->get->get('formid');
        $tableName = $this->getFabrikFormDBName($formid);
        $elementName = $this->getFabrikElementsName($formid);

        //var_dump($elementName)
        $attachId = $params->get('attachmentId');
        $sizeMax = $params->get('size');

        $name  = $this->getFullName(true, false);
        //var_dump($name).die();
        $element = $this->getElement();
        $file = $jinput->post->get($name);

        $url = 'index.php?option=com_fabrik&view=form&formid=' . $formid . '&Itemid=' . $itemid . '&usekey=fnum&rowid=' . $fnum;
        ...
        $target = EMUNDUS_PATH_ABS . $uid . DS . basename($file);

        $extension = explode('.', $fileName);
        $extensionAttachment = $attachmentResult->allowed_types;
        $typeExtension = $extension[1];

        //var_dump(stristr($extensionAttachment, $typeExtension)).die();
        ...
        if (stristr($extensionAttachment, $typeExtension) != false) {
            //if ($size <= $sizeMax) {


                //var_dump('if'.$typeExtension);
                $_SESSION['error'] .= '';
                if (empty($uploadResult)) {
                    if ($attachmentResult->nbmax >= 1) {
                        $this->insertFile($uid, $fnum, $cid, $attachId, $fileName);
                    }
                }
                if (!empty($uploadResult)) {
                    if ($attachmentResult->nbmax = 1) {
                        $this->updateFile($fnum, $cid, $attachId, $fileName);
                    } else {
                        $this->insertFile($uid, $fnum, $cid, $attachId, $fileName);
                    }
                }
            ...
            $app->enqueueMessage('Le format de votre fichier pour '.$element->label.' n\'est pas celui exigé');
            //var_dump($this->validationError).die();
            $app->redirect($url);

        }
        if (isset($_POST['delete'])) {
            if ($uploadResult->can_be_viewed = 1) {
                $this->deleteFile($fnum,$cid,$attachId);
                unlink($target);
            }
        }
    ...
    public function getFabrikElementsName($formid) {
        $db = JFactory::getDBO();

        $query = 'SELECT element.name, element.params
                FROM #__fabrik_elements AS element
                INNER JOIN #__fabrik_groups AS groups ON element.group_id = groups.id
                INNER JOIN #__fabrik_formgroup AS formgroup ON groups.id = formgroup.group_id
                INNER JOIN #__fabrik_forms AS form ON formgroup.form_id = form.id
                WHERE element.plugin LIKE "emundus_fileupload"
                AND form.id = '.$formid;
    ...
    public function getFabrikFormDBName($formid) {
        $db = JFactory::getDBO();
    ...
    public function insertFile($uid,$fnum,$cid,$attachId,$fileName){
        $db = JFactory::getDBO();
        $query = $db->getQuery(true);
        $columns = array('user_id', 'fnum', 'campaign_id', 'attachment_id', 'filename', 'can_be_deleted', 'can_be_viewed');
        $values = array($uid, $db->quote($fnum), $cid, $attachId, $db->quote($fileName), 0, 1);
        $query
            ->clear()
    ...
    public function deleteFile($fnum,$cid,$attachId){
        $db = JFactory::getDBO();
        $query = $db->getQuery(true);
