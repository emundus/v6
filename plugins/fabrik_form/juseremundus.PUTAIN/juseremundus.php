 ...
 * @subpackage  Fabrik.form.juseremundu
...
class PlgFabrik_FormJUseremundus extends plgFabrik_Form
{
	...
	public function onLoad($params, &$formModel)
	{
		if ($params->get('synchro_users') == 1)
		{
			...
			if ($count === 0)
			{
				...
				foreach ($origUsers as $o_user)
				{
					...
					$query->clear();
					$fields = array($this->getFieldName($params, 'juseremundus_field_userid', true) => $o_user->id,
						$this->getFieldName($params, 'juseremundus_field_block', true) => $o_user->block,
						$this->getFieldName($params, 'juseremundus_field_email', true) => $o_user->email,
						$this->getFieldName($params, 'juseremundus_field_password', true) => $o_user->password,
						$this->getFieldName($params, 'juseremundus_field_name', true) => $o_user->name,
						$this->getFieldName($params, 'juseremundus_field_username', true) => $o_user->username,
						$this->getFieldName($params, 'juseremundus_field_usertype', true) => $o_user->group_id);
		...
		{
			$this->passwordfield = $this->getFieldName($params, 'juseremundus_field_password');
			...
			{
				$this->useridfield = $this->getFieldName($params, 'juseremundus_field_userid');
				...
				if ($userid > 0)
				{
					...
					if ($user->get('id') == $userid)
					{
						...
						if ($params->get('juseremundus_field_usertype') != '')
						{
							...
							$this->gidfield = $this->getFieldName($params, 'juseremundus_field_usertype');
							$formModel->_data[$this->gidfield] = $gid;
							$formModel->_data[$this->gidfield . '_raw'] = $gid;
						}
						if ($params->get('juseremundus_field_block') != '')
						{
							$this->blockfield = $this->getFieldName($params, 'juseremundus_field_block');
	...
	public function onDeleteRowsForm($params, &$formModel, &$groups)
	{
		...
		{
			$useridfield = $this->getFieldName($params, 'juseremundus_field_userid');
			$useridfield .= '_raw';
	...
	public function onBeforeStore($params, &$formModel)
	{
		...
		if (!$this->shouldProcess('juseremundus_conditon', null, $formModel))
		{
			return true;
		}
		...
		$this->passwordfield = $this->getFieldName($params, 'juseremundus_field_password');
		$this->passwordvalue = $this->getFieldValue($params, 'juseremundus_field_password', $formModel->_formData);

		$this->namefield = $this->getFieldName($params, 'juseremundus_field_name');
		$this->namevalue = $this->getFieldValue($params, 'juseremundus_field_name', $formModel->_formData);

		$this->usernamefield = $this->getFieldName($params, 'juseremundus_field_username');
		$this->usernamevalue = $this->getFieldValue($params, 'juseremundus_field_username', $formModel->_formData);

		$this->emailfield = $this->getFieldName($params, 'juseremundus_field_email');
		$this->emailvalue = $this->getFieldValue($params, 'juseremundus_field_email', $formModel->_formData);
	 ...
	 * @param   object  $user       joomla user before juseremundu plugin run
	 *
	...
	protected function setGroupIds($formModel, $me, $user)
	{
		...
		$groupIds = (array) $this->getFieldValue($params, 'juseremundus_field_usertype', $formModel->_formData, $defaultGroup);
	...
	protected function autoLogin($formModel)
	{
		 ...
		 * com_fabrik.form.X.juseremundu.created with values true/false.
		...
		$session = JFactory::getSession();
		$context = 'com_' . $package . '.form.' . $formModel->getId() . '.juseremundu.';
		$w = new FabrikWorker;
