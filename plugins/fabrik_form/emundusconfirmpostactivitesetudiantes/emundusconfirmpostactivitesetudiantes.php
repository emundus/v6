<?php
/**
 * @version 2: emundusconfirmpostactivitesetudiantes 2018-09-06 Hugo Moracchini
 * @package Fabrik
...
class PlgFabrik_FormEmundusconfirmpostactivitesetudiantes extends plgFabrik_Form
{
	/**
	 * Status field
	 *
	...
	public function onAfterProcess() {
        ...
        $query->select('ee.id,ee.event_capacity,ee.waiting_list_capacity')
            ...
            ->where($db->quoteName('sc.id') . ' = ' . $fnumInfos['id']);
        $db->setQuery($query);
        $event = $db->loadObject();
        ...
        $query->clear()
            ...
            ->where($db->quoteName('event_id') . ' = ' . $db->quote($event->id))
            ->andWhere($db->quoteName('published') . ' = 1');
        $db->setQuery($query);
        $registrants_number = $db->loadResult();

        if($event->event_capacity == $registrants_number){
            $eb_status =  3;
            $status =  6;
        } else {
            $eb_status =  1;
        ...
        while (true)
        {
            $uniqueCode = JUserHelper::genRandomPassword(16);
            $query->clear()
                ->select('COUNT(*)')
                ->from('#__eb_registrants')
                ->where($db->quoteName('ticket_code') . ' = ' . $db->quote($uniqueCode));
            $db->setQuery($query);
            $total = $db->loadResult();
        ...
        $columns = ['event_id', 'user_id', 'first_name', 'last_name', 'email', 'number_registrants', 'register_date', 'payment_date', 'published', 'language', 'user_ip', 'ticket_code'];
        $values = [$event->id, $fnumInfos['applicant_id'], $db->quote($fnumInfos['name']), $db->quote($fnumInfos['name']), $db->quote($fnumInfos['email']), 1, $db->quote(date('Y-m-d h:i:s')), $db->quote(date('Y-m-d h:i:s')), $eb_status, $db->quote('fr-FR'), $db->quote($ip), $db->quote($uniqueCode)];
