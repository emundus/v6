...
if ($jinput->get('view') == 'form') {
	...
	if ($application_fee) {
		...
		if (count($fnumInfos) > 0) {
			...
			if ($accept_other_payments) {
                $payment_created_offline = (!(array)$m_application->getHikashopOrder($fnumInfos,)) ? 0 : 1;
            }
			else {
			...
			if ($accept_other_payments != 2 && !$paid && !$payment_created_offline && $attachments >= 100 && $forms >= 100) {
