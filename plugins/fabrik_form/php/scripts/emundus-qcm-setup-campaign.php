...
try {
    ...
    if((int)$data->type === 2){
        ...
        foreach ($data->categories as $category){
            ...
            // Create menu
            $new_menu = $m_formbuilder->createMenu($label_section,'',$data->profile,false);
            ...
            $values = array($db->quote($new_menu['id']), $db->quote($group_id), 0);
            ...
            $columns = array('list_id', 'element_id', 'join_from_table', 'table_join', 'table_key', 'table_join_key', 'join_type', 'group_id', 'params');
            $values = array($list_id, 0, $new_menu['db_table_name'], $new_menu['db_table_name'] . '_'.$group_id.'_repeat', 'id', 'parent_id', 'left', $group_id, '{"type":"group","pk":"`'.$new_menu['db_table_name'].'_'.$group_id.'_repeat`.`id`"}');
            foreach ($columns as $key => $column) {
                $columns[$key] = $db->quoteName($column);
            }
            foreach ($values as $key => $value) {
            ...
            $columns = array($db->quoteName('form_id'), $db->quoteName('group_id'), $db->quoteName('ordering'));
            $values = array($db->quote($new_menu['id']), $db->quote($total_group_id), 0);
            $query->clear()
            ...
            try {
                ...
                $values = array(0, 'QCM - ' . $new_menu['id'], '', null, 1, 'content-bottom-a', 0, date('Y-m-d H:i:s'), date('Y-m-d H:i:s'), '2029-04-09 13:02:54', 1, 'mod_emundus_qcm', 1, 0, json_encode($params), 0, '*');
                foreach ($columns as $key => $column) {
                    $columns[$key] = $db->quoteName($column);
                }
                foreach ($values as $key => $value) {
