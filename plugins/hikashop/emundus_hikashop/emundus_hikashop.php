    ...
    public function onCheckoutWorkflowLoad(&$checkout_workflow, &$shop_closed, $cart_id) {
        ...
        if (!empty($session)) {
            ...
            if (!empty($fnum)) {
                ...
                $query->select('jecc.wish_number', 'jecc.campaign_id')
                ...
                try {
                    ...
                    JLog::add('Unable to get file from ' . $fnum . ' ' . $e->getMessage(), JLog::ERROR, 'com_emundus.error');
                }

                if (!empty($file) && !empty($file->wish_number)) {
                    if ($file->wish_number != 1) {
                        $app->enqueueMessage(JText::_('MUST_SEND_WISHES_IN_ORDER'), 'warning');
                        $app->redirect('/');
                    }
                    // continue...
                } else {
                    $app->enqueueMessage(JText::_('COM_EMUNUDS_NO_WISH_NUMBER_FOUND') . $query->__toString(), 'error');
                    $app->redirect('/'); 
                }
            } else {
                $app->enqueueMessage(JText::_('COM_EMUNUDS_HIKASHOP_EMPTY_SESSION'), 'error');
                $app->redirect('/');
            }
        } else {
            $app->enqueueMessage(JText::_('COM_EMUNUDS_HIKASHOP_EMPTY_SESSION'), 'error');
