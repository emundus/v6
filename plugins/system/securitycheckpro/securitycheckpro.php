...
class plgSystemSecuritycheckpro extends JPlugin
{
	...
	function is_base64($value)
	{
		...
		if ($min_len) {
			...
			}else
			{
				...
				{
					if ((empty($caracter)) || (ord($caracter)<32) || (ord($caracter)>126))
		...
		if (!$modified)
		{
			...
			$usingIntegers = array("/0x(?=[0-9])/i");
				...
				if (!(strstr($escape_strings_exceptions,$pageoption)) && !(strstr($escape_strings_exceptions,'*')) && ($modified))
				{
					...
					if (strcmp($string_sanitized,$string) <> 0)
					{ //Se han añadido barras invertidas a ciertos caracteres; escribimos en el log							
						if ($base64)
						{
	...
	function check_header_and_user_agent($logs_attacks,$user,$user_agent,$referer,$ip,$methods_options,$a,$request_uri,$sqlpatterns,$ifStatements,$usingIntegers,$lfiStatements,$username,$pageoption)
	{
		...
		if ($user->guest)
		{
			...
			if (isset($_SERVER['HTTP_USER_AGENT']))
			{
				...
				if (strcmp($header_sanitized,$user_agent) <> 0)
				{ //Se han eliminado caracteres; escribimos en el log
			...
			if (!$modified)
			{
				if (isset($_SERVER['HTTP_REFERER']))
				{
					...
					if (strcmp($header_sanitized,$referer) <> 0)
					{ //Se han eliminado caracteres; escribimos en el log
	...
	function second_level($request_uri,$string,$a,&$found,$option)
	{
		...
		{ 
			if ((!($is_admin)) && ($pageoption != '') && !(is_array($string)))
			{  // No estamos en la parte administrativa
	...
	function mandar_correo($alerta)
	{
		...
		{  // No se ha alcanzado el límite máximo de emails por día
			/* Cargamos el lenguaje del sitio */
			$lang = JFactory::getLanguage();
			$lang->load('com_securitycheckpro',JPATH_ADMINISTRATOR);
	...
	protected function sesiones_activas($logs_attacks,$attack_ip,$request_uri,$session_protection_active,$session_hijack_protection)
	{
		...
		{			
			...
			if (($result > 1) && ($apply_to_user))
			{  // Ya existe más de una sesión activa del usuario y el usuario está incluido en un grupo al que hay que aplicar la protección				
	...
	private function forbid_new_admins()
	{
		...
		if ($forbid_new_admins)
		{
			...
			{ // No hay datos almacenados (o es la primera vez que se lanza o se ha desactivado esta opción y ahora está activa)
