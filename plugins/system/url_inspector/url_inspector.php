...
class PlgSystemurl_Inspector extends JPlugin
{
	...
	public function __construct(&$subject, $config)
	{
		...
		JError::setErrorHandling(E_ERROR, 'callback', array('PlgSystemurl_Inspector', 'handleError'));

		// Register the previously defined exception handler so we can forward errors to it
		self::$previousExceptionHandler = set_exception_handler(array('PlgSystemurl_Inspector', 'handleException'));
	}
	...
	private static function doErrorHandling($error)
	{
		...
		/*if ($app->isAdmin() || ((int) $error->getCode() !== $url))
		{
