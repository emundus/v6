...
class PlgDPCalendarDPCalendar_Jomsocial extends DPCalendarPlugin
{
	...
	private function writeOnStream($action, $event)
	{
		...
		$act->target = 0;
		$act->title = JText::sprintf('PLG_DPCALENDAR_JOMSOCIAL_ACTIVITY_' . strtoupper($action), $event->title);
		$act->content = '';
		$act->app = 'wall';
		$act->location = DPCalendarHelperLocation::format($event->locations);
		$act->cid = $event->id;

		$params = new CParameter('');
		$params->set('eventid', $event->id);
		$params->set('action', 'dpevents.' . $action);
		$params->set('event_url', 'index.php?option=com_dpcalendar&view=event&id=' . $event->id);
