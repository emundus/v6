...
JLoader::import('dpcalendar.dpcalendar_meetup.libraries.meetup.meetup', JPATH_PLUGINS);

class PlgDPCalendarDPCalendar_Meetup extends DPCalendarSyncPlugin
{
	...
	protected function refresh (&$calendar)
	{
		...
		if (time() >= (intval($calendar->params->get('expires')) - 60))
		{
			...
			try
			{
				...
				$table->bind(array(
						'params' => $calendar->params->toString()
				));
	...
	public function import ()
	{
		...
		$status = $session->get('status', array(
				'past',
				'upcoming'
		), $this->_name);
