...
class PlgDPCalendarDPCalendar_Google extends DPCalendarSyncPlugin
{
	...
	public function saveEvent ($eventId = null, $calendarId, array $data)
	{
		...
		try
		{
			...
			{
				$event->setLocation(DPCalendarHelperLocation::format(DPCalendarHelperLocation::getLocations($data['location_ids'])));
			}
	...
	private function createEventFromGoogle ($googleEvent, $calendar, $obj, $colors)
	{
		...
		if ($googleEvent->location)
		{
			$event->locations = array(
					DPCalendarHelperLocation::get($googleEvent->location)
			);
		}
