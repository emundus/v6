...
class EmundusHelperUpdate
{
	...
	public static function manageCcid()
	{
		...
		foreach ($tables as $table)
		{
			...
			/* If the table doesn't have a ccid column, we add it with index and we fill it */
			...
			/* Now we have to create ccid fabrik element for each forms linked to this table */
			$query = $db->getQuery(true);
			$query->select('form_id')
				->from($db->quoteName('#__fabrik_lists'))
				->where($db->quoteName('db_table_name') . ' = ' . $db->quote($table));
			$db->setQuery($query);
			$forms = $db->loadColumn();
			...
			if (!empty($forms))
			{
				$query->clear()
					...
					->leftJoin($db->quoteName('#__fabrik_formgroup', 'ff') . ' ON ' . $db->quoteName('fe.group_id') . ' = ' . $db->quoteName('ff.group_id'))
					->where($db->quoteName('ff.form_id') . ' IN (' . implode(',', $forms) . ')')
					->where($db->quoteName('fe.name') . ' LIKE ' . $db->quote('ccid'));
				$db->setQuery($query);
				$ccid_existing = $db->loadResult();

				if (empty($ccid_existing))
				{
					/* We need to find the group_id of the fnum element */
					$query->clear()
						->select('fe.group_id')
						->from($db->quoteName('#__fabrik_elements', 'fe'))
						->leftJoin($db->quoteName('#__fabrik_formgroup', 'ff') . ' ON ' . $db->quoteName('fe.group_id') . ' = ' . $db->quoteName('ff.group_id'))
						->where($db->quoteName('ff.form_id') . ' IN (' . implode(',', $forms) . ')')
						->where($db->quoteName('fe.name') . ' LIKE ' . $db->quote('fnum'));
					$db->setQuery($query);
					$fnum_group = $db->loadResult();
		...
		/* We need to find all tables that have a ccid column and add a foreign key to campaign_candidature */
