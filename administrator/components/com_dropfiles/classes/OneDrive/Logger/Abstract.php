 ...
 */
abstract class OneDrive_Logger_Abstract
{
  /**
   * Default log format
   */
  ...
  /**
   * @param OneDrive_Client $client  The current OneDrive client
   */
  public function __construct(OneDrive_Client $client)
  {
    $this->setLevel(
        $client->getClassConfig('OneDrive_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('OneDrive_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('OneDrive_Logger_Abstract', 'date_format');
    $this->dateFormat = $format ? $format : self::DEFAULT_DATE_FORMAT;

    $this->allowNewLines = (bool) $client->getClassConfig(
        'OneDrive_Logger_Abstract',
        'allow_newlines'
    );
  }
   ...
   * @throws OneDrive_Logger_Exception If $level is invalid
   */
  protected function normalizeLevel($level)
  {
    ...
    throw new OneDrive_Logger_Exception(
