 ...
 */
class OneDrive_Logger_File extends OneDrive_Logger_Abstract
{
  /**
  ...
  public function __construct(OneDrive_Client $client)
  {
    ...
    $file = $client->getClassConfig('OneDrive_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('OneDrive_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('OneDrive_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new OneDrive_Logger_Exception('File pointer is no longer available');
    }
