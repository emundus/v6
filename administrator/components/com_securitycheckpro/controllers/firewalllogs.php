...
*
*/
class SecuritycheckprosControllerFirewallLogs extends SecuritycheckproController
{

/* Redirecciona las peticiones al componente */
function redireccion()
{
	$this->setRedirect( 'index.php?option=com_securitycheckpro&controller=firewalllogs&view=firewallcpanel' );
}
...
{
	$model = $this->getModel('firewalllogs');
	$data = JRequest::get('post');
	
	if ( !is_numeric($data['log_limits_per_ip_and_day']) ) {
		JFactory::getApplication()->enqueueMessage(JText::_('COM_SECURITYCHECKPRO_INVALID_VALUE'),'error');
	} else {
		$model->saveConfig($data, 'pro_plugin');
	}
	
	$this->setRedirect('index.php?option=com_securitycheckpro&view=firewalllogs&'. JSession::getFormToken() .'=1');
}
