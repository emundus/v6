...
class DPCalendarControllerBooking extends JControllerForm
{
	...
	public function invoicesend()
	{
		...
		$subject = DPCalendarHelper::renderEvents(array(
				$event
		), JText::_('COM_DPCALENDAR_BOOK_NOTIFICATION_SEND_SUBJECT'), null, $additionalVars);
		$body = DPCalendarHelper::renderEvents(array(
				$event
		), JText::_('COM_DPCALENDAR_BOOK_NOTIFICATION_SEND_BODY'), null, $additionalVars);
		...
		$fileName = DPCalendarHelperBooking::createInvoice($booking, $tickets, $state->params, true);
		if ($fileName)
		{
