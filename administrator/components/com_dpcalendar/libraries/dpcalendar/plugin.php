...
abstract class DPCalendarPlugin extends JPlugin
{
	...
	protected function fetchCalendars ($calendarIds = null)
	{
		if ($this->extCalendarsCache === null)
		{
			...
			$this->extCalendarsCache = JModelLegacy::getInstance('Extcalendars', 'DPCalendarModel', array(
					'ignore_request' => true
			));
		}
	...
	private function createEventFromIcal (Sabre\VObject\Component\VEvent $event, $calendarId, array $originals)
	{
		...
		$tmpEvent->title = DPCalendarHelperIcal::icalDecode($title);
		$tmpEvent->alias = JApplication::stringURLSafe($tmpEvent->title);
		$tmpEvent->description = DPCalendarHelperIcal::icalDecode((string) $event->DESCRIPTION);
		...
		if (! empty($description))
		{
			...
			$tmpEvent->description = DPCalendarHelperIcal::icalDecode($desc);
		}
		...
		if (! empty($image))
		{
			$tmpEvent->images = json_encode(array(
					'image1' => $image
			));
		}
		...
		if (! empty($location))
		{
			...
			if (! empty($geo) && strpos($geo, ';') !== false)
			{
				...
				{
					list ($latitude, $longitude) = explode(';', $geo);
					$locationObject = DPCalendarHelperLocation::get($latitude . ',' . $longitude);
			...
			{
				$locations[] = DPCalendarHelperLocation::get(DPCalendarHelperIcal::icalDecode($location));
			}
		}
	...
	protected function matchLocationFilterEvent($event, Registry $options)
	{
		...
		{
			if (!in_array($loc->id, $locationIds) && !DPCalendarHelperLocation::within($loc, $locationFilterData->latitude, $locationFilterData->longitude, $radius))
			{
				continue;
			}
			$within = true;
