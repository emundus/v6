...
class DPCalendar extends CalDAV\Backend\PDO
{
	...
	public function getCalendarsForUser($principalUri)
	{
		...
		foreach (\DPCalendarHelper::getCalendar('root')->getChildren(true) as $calendar)
		{
			...
			$calendars[] = array(
					...
					'{http://calendarserver.org/ns/}getctag' => $params->get('etag', 1),
	...
	public function getCalendarObject($calendarId, $objectUri)
	{
		...
		if (!empty($event->id))
		{
			// The event needs to be loaded trough the model to get locations,
			// tags, etc.
	...
	public function deleteCalendarObject($calendarId, $objectUri)
	{
		if (strpos($calendarId, 'dp-') !== false)
		{
			...
			$calendar = \DPCalendarHelper::getCalendar(str_replace('dp-', '', $calendarId));
			if (!$calendar || !$calendar->canDelete)
			{
	...
	private function merge($dpEvent, $vEvent)
	{
		...
		if (isset($vEvent->LOCATION) && $vEvent->LOCATION->getValue())
		{
			...
			if (!$location)
			{
				...
				{
					if ($l->title == $locationString || $l->alias == $locationString || \DPCalendarHelperLocation::format($l) == $locationString)
					{
						$location = $l;
						break;
					}
				...
				if (!$location)
				{
					$location = \DPCalendarHelperLocation::get($locationString);
				}
			}
			if ($location)
			{
