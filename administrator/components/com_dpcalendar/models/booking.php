...
class DPCalendarModelBooking extends JModelAdmin
{
	...
	public function save($data, $invite = false)
	{
		...
		if (!array_key_exists('id', $data) || !$data['id'])
		{
			...
			foreach ((array)$data['event_id'] as $eId => $amount)
			{
				...
				{
					// Free event
					$event->amount_tickets[0] = $amount[0] > $event->max_tickets ? $event->max_tickets : $amount[0];
				...
				else
				{
					...
					{
						// Check if the user or email address has already
						// tickets booked
						$bookedTickets = 0;
						foreach ($event->tickets as $ticket)
						{
							if (($ticket->email !== $data['email'] && ($user->guest || $ticket->user_id != $data['user_id'])) ||
									 $ticket->type != $index || $event->amount_tickets[$index] < 1)
							{
								continue;
							}
							$bookedTickets ++;
						...
						$event->amount_tickets[$index] = $amount[$index] > $event->max_tickets - $bookedTickets ? $event->max_tickets - $bookedTickets : $amount[$index];

						if ($event->amount_tickets[$index] < 1)
						{
							$event->amount_tickets[$index] = 0;
							$app->enqueueMessage(
						...
						// Determine the price
						$paymentRequired = DPCalendarHelperBooking::paymentRequired($event);
						if ($paymentRequired)
						{
							// Set state to payment required
							$data['state'] = 3;

							// Determine the price based on the amount of
							// tickets
							$data['price'] += DPCalendarHelperBooking::getPriceWithDiscount($value, $event) * $event->amount_tickets[$index];
						}
					}
				}
		...
		JFactory::getApplication()->input->set('b_id', $id);
		...
		{
			$ticketModel = JModelLegacy::getInstance('Ticket', 'DPCalendarModel', array(
					'ignore_request' => true
			));
			foreach ($events as $event)
			{
				...
				foreach ($prices->value as $index => $value)
				{
					for ($i = 0; $i < $event->amount_tickets[$index]; $i ++)
					{
						...
						$ticket->price = DPCalendarHelperBooking::getPriceWithDiscount($value, $event);
		...
		else if ($oldItem && $oldItem->state != $item->state)
		{
		...
		if ($this->getState($this->getName() . '.new'))
		{
			// If a payment is required include the payment statement from the
			// plugin
			$oldDetails = $additionalVars['bookingDetails'];
			if ($item->state == 3 || $item->state == 4)
			{
				$additionalVars['bookingDetails'] = $additionalVars['bookingDetails'] . '<br/>' .
						 DPCalendarHelperBooking::getPaymentStatementFromPlugin($item);
			}

			// Send a mail to the booker
			$subject = DPCalendarHelper::renderEvents($events, JText::_('COM_DPCALENDAR_NOTIFICATION_EVENT_BOOK_USER_SUBJECT'), null, $additionalVars);
			$body = trim(DPCalendarHelper::renderEvents($events, JText::_('COM_DPCALENDAR_NOTIFICATION_EVENT_BOOK_USER_BODY'), null, $additionalVars));
			...
			if (!empty($body))
			{
				...
				$content = DPCalendarHelperIcal::createIcalFromEvents($events, false);
		...
		else if ($oldItem && ($oldItem->state == 3 || $oldItem->state == 4) && $item->state == 1)
		{
			// We have a successfull payment
			$subject = DPCalendarHelper::renderEvents(array(), JText::_('COM_DPCALENDAR_NOTIFICATION_EVENT_BOOK_USER_PAYED_SUBJECT'), null,
					$additionalVars);
			$body = DPCalendarHelper::renderEvents(array(), JText::_('COM_DPCALENDAR_NOTIFICATION_EVENT_BOOK_USER_PAYED_BODY'), null, $additionalVars);
			...
			$fileName = DPCalendarHelperBooking::createInvoice($item, $tickets, $params, true);
			if ($fileName)
			{
				$mailer->addAttachment($fileName);
			}
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		$form = $this->loadForm('com_dpcalendar.booking', 'booking', array(
				'control' => 'jform',
				'load_data' => $loadData
		));
		if (empty($form))
	...
	protected function loadFormData()
	{
		...
		if (!$data->id && !JFactory::getUser()->guest)
		{
			$this->getDbo()->setQuery(
					'select id from #__dpcalendar_bookings where user_id = ' . JFactory::getUser()->id . ' order by id desc limit 1');
	...
	protected function populateState()
	{
		...
		$pk = $app->input->getInt('b_id', JFactory::getApplication()->isAdmin() ? $app->input->getInt('id') : null);
