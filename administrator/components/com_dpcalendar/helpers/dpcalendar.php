...
JLoader::register('DPCalendarHelperIcal', JPATH_ADMINISTRATOR . '/components/com_dpcalendar/helpers/ical.php');
JLoader::register('DPCalendarHelperLocation', JPATH_ADMINISTRATOR . '/components/com_dpcalendar/helpers/location.php');

JLoader::register('DPCalendarHelperBooking', JPATH_ADMINISTRATOR . '/components/com_dpcalendar/helpers/booking.php');
...
class DPCalendarHelper
{
	...
	public static function addSubmenu ($vName = 'cpanel')
	{
		...
		JHtmlSidebar::addEntry(JText::_('COM_DPCALENDAR_SUBMENU_CALENDARS'), 'index.php?option=com_categories&extension=com_dpcalendar',
				$vName == 'categories');
		...
		if (! self::isFree())
		{
			...
			JHtmlSidebar::addEntry(JText::_('COM_DPCALENDAR_SUBMENU_BOOKINGS'), 'index.php?option=com_dpcalendar&view=bookings', $vName == 'bookings');
		}
		if (self::existsLibrary('com_dpfields'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_DPCALENDAR_SUBMENU_FIELDS'), 'index.php?option=com_dpfields&context=com_dpcalendar.event',
					$vName == 'fields.event' || $vName == 'fields.location' || $vName == 'fields.booking' || $vName == 'fields.ticket');

			$ex = JFactory::getApplication()->input->getCmd('extension');
			JHtmlSidebar::addEntry(JText::_('COM_DPCALENDAR_SUBMENU_FIELD_CATEGORIES'),
					'index.php?option=com_categories&extension=com_dpcalendar.event.fields',
					$ex == 'com_dpcalendar.event.fields' || $ex == 'com_dpcalendar.location.fields' || $ex == 'com_dpcalendar.booking.fields' ||
							 $ex == 'com_dpcalendar.ticket.fields');
		}
	...
	public static function renderEvents (array $events = null, $output, $params = null, $eventParams = array())
	{
		...
		foreach ($events as $event)
		{
			...
			$variables['canBook'] = DPCalendarHelperBooking::openForBooking($event);
	...
	{
		return JLayoutHelper::render('format.price', array('price' => $price, 'currency' => $currencySymbol));
	}
	...
	public static function exportCsv ($name, $fieldsToLabels, $valueParser)
	{
		...
		$model = JModelLegacy::getInstance(ucfirst($name) . 's', 'DPCalendarModel', array(
		...
		if ($items)
		{
			...
			{
				$fields = DPFieldsHelper::getFields('com_dpcalendar.' . $name);
		...
		foreach ($items as $item)
		{
			...
			if ($fields)
			{
				foreach ($fields as $field)
				{
					if (! isset($item->dpfields) || ! key_exists($field->id, $item->dpfields))
					{
						continue;
					}
					$line[] = strip_tags($item->dpfields[$field->id]->value);
				}
			}
			fputcsv($f, $line, $delimiter);
		}
	...
	public static function isCaptchaNeeded ()
	{
		...
		$user = JFactory::getUser();
		$accessCaptcha = array_intersect(self::getComponentParameter('captcha_groups', array(
				1
	...
	public static function loadLibrary ($libraries = array('jquery' => true))
	{
		...
		if (isset($libraries['maps']))
		{
			...
			$document->addScript(
					(JBrowser::getInstance()->isSSLConnection() ? "https" : "http") . '://maps.googleapis.com/maps/api/js?libraries=places&language=' .
							 self::getGoogleLanguage() . $key);
		}
	...
	public static function existsLibrary ($library)
	{
		switch ($library)
		{
			case 'com_dpfields':
				return JFolder::exists(JPATH_ADMINISTRATOR . '/components/com_dpfields');
			default:
				return false;
		}
	}
