...
class LiveUpdateController extends JControllerLegacy
{
	...
	public function startupdate()
	{
		...
		{
			$skipNag = JFactory::getApplication()->input->getBool('skipnag', false);
			if (!$skipNag)
			{
				$this->setRedirect('index.php?option=' . JFactory::getApplication()->input->get('option', '') . '&view=' . JFactory::getApplication()->input->get('view', 'liveupdate') . '&task=nagscreen');
				$this->redirect();
			}
		}
		...
		else
		{
			...
			$this->setRedirect('index.php?option=' . JFactory::getApplication()->input->get('option', '') . '&view=' . JFactory::getApplication()->input->get('view', 'liveupdate') . '&task=download');
			$this->redirect();
		}
	}
	...
	public function download()
	{
		...
		catch (LiveUpdateDownloadException $e)
		{
			...
			if ($config->requiresAuthorization())
			{
				...
				$this->setRedirect('index.php?option=' . JFactory::getApplication()->input->get('option', '') . '&view=' . JFactory::getApplication()->input->get('view', 'liveupdate') . '&task=overview', $msg, 'error');
		...
		else
		{
			...
			$url = 'index.php?option=' . JFactory::getApplication()->input->get('option', '') . '&view=' . JFactory::getApplication()->input->get('view', 'liveupdate') . '&task=extract';
	...
	public function extract()
	{
		...
		else
		{
			...
			$url = 'index.php?option=' . JFactory::getApplication()->input->get('option', '') . '&view=' . JFactory::getApplication()->input->get('view', 'liveupdate') . '&task=install';
			$user = JFactory::getApplication()->input->getString('username', null, 'GET', JREQUEST_ALLOWRAW);
			$pass = JFactory::getApplication()->input->getString('password', null, 'GET', JREQUEST_ALLOWRAW);
			if ($user)
			{
	...
	public function install()
	{
		...
		if (!$result)
		{
			// Installation failed
			$model->cleanup();
			$this->setRedirect('index.php?option=' . JFactory::getApplication()->input->get('option', '') . '&view=' . JFactory::getApplication()->input->get('view', 'liveupdate') . '&task=overview');
			$this->redirect();
		}
		else
		{
	...
	private function setCredentialsFromRequest($client)
	{
		...
		if ($user != '' && $pass != '')
		{
			...
			{
                Factory::getApplication()->enqueueMessage(JText::_('JClientHelper::setCredentialsFromRequest failed'), 'warning');
                $return = false;
