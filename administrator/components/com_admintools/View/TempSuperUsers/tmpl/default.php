...
<form action="index.php" method="post" name="adminForm" id="adminForm" class="akeeba-form">
		...
		<div class="akeeba-filter-bar akeeba-filter-bar--left akeeba-form-section akeeba-form--inline">
			<div class="akeeba-filter-element akeeba-form-group">
				<input type="text" name="username"
					   placeholder="<?php echo Text::_('COM_ADMINTOOLS_TEMPSUPERUSERS_LBL_USERNAME'); ?>"
					   id="filter_username" onchange="document.adminForm.submit();"
					   value="<?php echo $this->escape($this->filters['username']); ?>"
					   title="<?php echo Text::_('COM_ADMINTOOLS_TEMPSUPERUSERS_LBL_USERNAME'); ?>" />
			</div>
		</div>
	...
	<table class="akeeba-table akeeba-table--striped" id="itemsList">
		...
		<tr>
			...
			<th>
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_TEMPSUPERUSERS_FIELD_USER_ID', 'user_id', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
			<th>
				<?php echo Text::_('COM_ADMINTOOLS_TEMPSUPERUSERS_LBL_USERNAME') ?>
			</th>
			<th>
				<?php echo HTMLHelper::_('grid.sort', 'COM_ADMINTOOLS_TEMPSUPERUSERS_FIELD_EXPIRATION', 'expiration', $this->order_Dir, $this->order, 'browse'); ?>
			</th>
		</tr>
		</thead>
		<tfoot>
		...
		<?php if (!count($this->items)): ?>
			<tr>
				<td colspan="6">
					<?php echo Text::_('COM_ADMINTOOLS_ERR_TEMPSUPERUSERS_NOITEMS') ?>
				</td>
			</tr>
			<tr>
				<td colspan="6">
					...
					>
						...
						<?php echo Text::_('COM_ADMINTOOLS_BTN_TEMPSUPERUSERS_ADD') ?>
					</a>
				</td>
			</tr>
		<?php endif; ?>
		...
		if ($this->items):
			$i = 0;
			/** @var TempSuperUsers $row */
			foreach ($this->items as $row):
				?>
				<tr>
					<td><?php echo HTMLHelper::_('grid.id', ++$i, $row->user_id); ?></td>
					<td>
					...
					<td>
						<?php if (empty($row->user)): ?>
							<span class="akeeba-label--red hasPopover"
								  data-title="<?= Text::_('COM_ADMINTOOLS_TEMPSUPERUSERS_LBL_INVALIDUSER') ?>"
								  data-content="<?= Text::_('COM_ADMINTOOLS_TEMPSUPERUSERS_LBL_INVALIDUSER_TIP') ?>">
					...
					<td>
						<?php echo Html::localisedDate($row->expiration) ?>
					</td>
				</tr>
			<?php
			endforeach;
