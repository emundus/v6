	...
	function akeeba_common_phpversion_warning($config = array())
	{
		...
		if ($isTooNew): ?>
			...
			<div class="{{ $class_priority_medium }}">
				...
				<p>
					Your site is currently using PHP {{ $longVersion }}. This version of PHP not supported by
					your currently installed version of {{ $softwareName }}. We cannot guarantee that
					{{ $softwareName }} will work correctly on your site.
				</p>
				<p>
					Please check our site for an updated version of {{ $softwareName }} which supports PHP
					{{ $shortVersion }}. If none is available yet you will need to wait 1-3 months. Meanwhile,
					you can still use PHP {{ $recommendedPHPVersion }} which is fully supported by
					{{ $softwareName }}. You can ask your host or your system administrator for instructions.
				</p>
			</div>
		@elseif($isAncient)
			<!-- Your PHP version has been End-of-Life for a very long time -->
			<div class="{{ $class_priority_high }}">
				<h3>Severely outdated PHP version {{ $shortVersion }}</h3>

				<p>
					Your site is currently using PHP {{ $longVersion }}. This version of PHP has become <a
						href="https://php.net/eol.php" target="_blank">End-of-Life since {{ $eolDateFormatted }}</a>. It has
					not received security updates for a <em>very long time</em>. You MUST NOT use it for a live site!
				</p>
				<p>
					{{ $softwareName }} will stop supporting your version of PHP very soon. You must <strong>very
						urgently</strong> upgrade to a newer version of PHP. We recommend PHP {{ $recommendedPHPVersion }}
		...
		elseif ($isEol):
			...
			<div class="{{ $class_priority_medium }}">
				...
				<p>
					Your site is currently using PHP {{ $longVersion }}. This version of PHP has recently become <a
						href="https://php.net/eol.php" target="_blank">End-of-Life since {{ $eolDateFormatted }}</a>. It has
		...
		elseif ($warn_about_maintenance):
			...
			<div class="{{ $class_priority_low }}">
				<h3>Older PHP version {{ $shortVersion }}</h3>

				<p>
					Your site is currently using PHP {{ $longVersion }}. This version of PHP has entered its
					“Security maintenance” phase since {{ $securityDateFormatted }} and has stopped receiving bug fixes. It
					will stop receiving security updates on {{ $eolDateFormatted }} at which point it will be unsuitable for
					use on a live site.
				</p>
				<p>
					{{ $softwareName }} will stop supporting your version of PHP soon after it becomes End-of-Life on
					{{ $eolDateFormatted }}. We recommend that you plan your migration to a newer version of PHP before that
					date. We recommend PHP {{ $recommendedPHPVersion }} or later. You can ask your host or your system
