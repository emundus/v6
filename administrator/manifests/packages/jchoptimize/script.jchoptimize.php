...
use JchOptimize\Component\Admin\Model\Utility;
...
class Pkg_JchoptimizeInstallerScript
{
	...
	protected $extensionsToEnable = array(
		array( 'plugin', 'jchoptimize', 0, 'system' ),
		array( 'module', 'mod_jchmodeswitcher', 1, '' )
	);
	...
	public function preflight( $type, $parent )
	{
		...
		$file = JPATH_ADMINISTRATOR . '/manifests/packages/pkg_jchoptimize.xml';
		...
		if ( file_exists( $file ) )
		{
			...
			{
				$msg = '<p>You are trying to install the FREE version of JCH Optimize but you currently have the PRO version installed. You must uninstall the PRO version before you can install the FREE version.</p>';
				JLog::add( $msg, JLog::WARNING, 'jerror' );
	...
	public function postflight( $type, $parent )
	{
		...
		/**
		 * Gonna have to load FOF at this point
		 */
		@include_once( JPATH_LIBRARIES . '/fof40/include.php' );
		/** @var \FOF40\Container\Container $container */
		$oContainer = Container::getInstance( 'com_jchoptimize' );
		/** @var \FOF40\Factory\BasicFactory $oFactory */
		$oFactory = $oContainer->factory;
		$oFactory->setSection('admin');

		//Order the plugin properly
		if ( in_array( $type, [ 'install', 'update' ] ) )
		{
			try
			{
				/** @var Utility $model */
				$model = $oFactory->model( 'Utility' );
				$model->orderPlugins();
			}
			catch ( \Exception $e )
			{
	...
	private function uninstallFOF( $parent )
	{
		...
		if ( $dependencyCount )
		{
			...
			JLog::add( $msg, JLog::DEBUG, 'jerror' );
