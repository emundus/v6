...
syncApp.controller('mainCtrl', ['$scope', '$http', function($scope, $http)
{
	...
	{
		var postObj = {refId: $scope.contactToSync[index].contact.refId, index:$scope.contactToSync[index].contact.index, orgaId: $scope.contactToSync[index].orga.orgaId}
	...
	{
		var postObj = {refId: $scope.contactToSync[index].contact.refId, index:$scope.contactToSync[index].contact.index, contactId: $scope.contactToSync[index].contact.cId}

		$http.post('index.php?option=com_jcrm&task=syncs.synccontact', postObj).
			success(function(data)
			        {
	...
	{
		var postObj = {refId: $scope.contactToSync[index].contact.refId, index: $scope.contactToSync[index].contact.index}
	...
	$scope.ignoreAll = function()
	{
		...
		{
			$scope.showDime = true;
			$http.post('index.php?option=com_jcrm&task=syncs.ignoreall', $scope.contactToSync).success(function(data)
		                     {
	...
	{
		$scope.showDime = true;
		$http.post('index.php?option=com_jcrm&task=syncs.validall', $scope.contactToSync).success(function(data)
		                                                                                           {
			                                                                                          $scope.loadData($scope.arrayConf.currentPage);
		                                                                                           });
	}
