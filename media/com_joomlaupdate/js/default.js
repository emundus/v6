...
((Joomla, document) => {
  ...
  Joomla.submitbuttonUpload = () => {
    ...
    const confirmBackup = document.getElementById('joomlaupdate-confirm-backup'); // do field validation
  ...
  PreUpdateChecker.checkNextChunk = extensionsArray => {
    ...
    Joomla.request({
      ...
      onError(xhr) {
        ...
        Joomla.renderMessages(Joomla.ajaxErrorsMessages(xhr)); // Mark all pending extensions as errored out on the server side
