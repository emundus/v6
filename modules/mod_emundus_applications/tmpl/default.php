...
<?php if ($show_add_application && ($position_add_application == 0 || $position_add_application == 2) && $applicant_can_renew) : ?>
  <a id="add-application" class="btn btn-success" href="<?= JURI::base(); ?>index.php?option=com_fabrik&view=form&formid=102"><span class="icon-plus-sign"> <?= JText::_('ADD_APPLICATION_FILE'); ?></span></a>
<hr>
<?php endif; ?>
  ...
  <div class="row" id="row<?= $application->fnum; ?>">
    ...
    <div class="col-xs-12 col-md-6 main-page-file-info">
      ...
      <?php if($application->status<=1) : ?>
        ...
        <a id="trash" class="btn btn-danger btn-xs" onClick="deletefile('<?= $application->fnum; ?>');" href="#row<?= $attachments[$application->fnum]; ?>" title="<?= JText::_('DELETE_APPLICATION_FILE'); ?>"><i class="icon-trash"></i> </a>
  ...
  <hr>
    ...
    <div class="row" id="row<?= $application->fnum; ?>">
        ...
        <div class="col-xs-12 col-md-6 main-page-file-info">
            ...
            <a class="btn btn-secondary" disabled="disabled" data-toggle="tooltip" data-placement="top" title="<?= JText::_('MOD_EMUNDUS_APPLICATION_TOOLTIP_REMOVE_OR_ARCHIVE_FILES'); ?>"  role="button">
                <i class="folder open outline icon"></i> <?= JText::_('OPEN_APPLICATION'); ?>
            </a>

            <?php if( ((int)($attachments[$application->fnum])>=100 && $application->status==0 && !$is_dead_line_passed) || in_array($user->id, $applicants) ) : ?>
                <a class="btn btn-secondary" disabled="disabled" data-toggle="tooltip" data-placement="top" title="<?= JText::_('MOD_EMUNDUS_APPLICATION_TOOLTIP_REMOVE_OR_ARCHIVE_FILES'); ?>"><i class="icon-envelope"></i> <?= JText::_('SEND_APPLICATION_FILE'); ?></a>
            <?php endif; ?>
...
function deletefile(fnum){
  if (confirm("<?= JText::_('CONFIRM_DELETE_FILE'); ?>")) {
    document.location.href="<?= JRoute::_(JURI::base().'index.php?option=com_emundus&task=deletefile&fnum='); ?>"+fnum;
  }
}
