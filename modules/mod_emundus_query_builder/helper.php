...
class modEmundusQueryBuilderHelper {
	public function getModuleStat()
	{
        ...
        try {
			$db->setQuery("SELECT `jos_emundus_setup_groups_repeat_course`.`course` FROM `jos_emundus_groups` LEFT JOIN `jos_emundus_setup_groups_repeat_course` ON (`jos_emundus_groups`.`group_id` = `jos_emundus_setup_groups_repeat_course`.`parent_id` )");
	...
	public function changePublishedModuleAjax()
	{
		...
		try {
			...
			echo json_encode((object)['status' => false, 'msg' => $e->getMessage()]);
			exit;
		}
	}
	...
	{
		$title = addslashes(str_replace("'", " ", JFactory::getApplication()->input->post->getString('titleModule')));
	...
	{
		$nameGraph = str_replace("'", " ", str_replace("\"", " ", JFactory::getApplication()->input->post->getString('titleModule')));
		...
		$nameAxeX = addslashes(JFactory::getApplication()->input->post->getString('axeXModule'));
		$nameAxeY = addslashes(JFactory::getApplication()->input->post->getString('axeYModule'));
		$progModule = JFactory::getApplication()->input->post->getString('progModule');
		$yearModule = JFactory::getApplication()->input->post->getString('yearModule');
		$campaignModule = JFactory::getApplication()->input->post->getString('campaignModule');
		$idMenu = JFactory::getApplication()->input->post->getString('idMenu');
		...
		try {
			...
			$db->setQuery("SELECT jos_fabrik_lists.db_table_name FROM (jos_fabrik_elements INNER JOIN jos_fabrik_formgroup ON jos_fabrik_elements.group_id = jos_fabrik_formgroup.group_id) INNER JOIN jos_fabrik_lists ON jos_fabrik_formgroup.form_id=jos_fabrik_lists.form_id WHERE jos_fabrik_elements.id = ".$indicateur);
			...
			if($typeModule === "timeseries") {
				...
				left join `emundus_bdd`.`jos_emundus_personal_detail` `".$dbTableName."` on(`ecc`.`applicant_id` = `".$dbTableName."`.`user`)
				...
				$db->setQuery("SELECT plugin FROM jos_fabrik_elements WHERE id = ".$indicateur);
				$result = $db->loadAssoc();
				...
				if($plugin === "databasejoin") {
					...
					left join `emundus_bdd`.`".$paramElt['join_db_name']."` on ( `".$dbTableName."`.`".$elementName."` = `".$paramElt['join_db_name']."`.`".$paramElt['join_key_column']."`)
					where `".$dbTableName."`.`".$elementName."` is not null and `ecc`.`submitted` = 1
					group by `".$dbTableName."`.`".$elementName."`");
					$db->execute();
			...
			$db->setQuery("SELECT ordering FROM jos_modules WHERE module LIKE 'mod_emundus_stat' OR module LIKE 'mod_emundus_stat_filter' OR module LIKE 'mod_emundus_query_builder' ORDER BY ordering DESC LIMIT 1");
			...
			$db->setQuery("INSERT INTO `jos_modules_menu` (`moduleid`, `menuid`) VALUES ('".$idModule."', '".$idMenu."')");
			$db->execute();
