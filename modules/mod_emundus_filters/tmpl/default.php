...
<section id="mod_emundus_filters">
    <span id="reset-all-filters" class="material-icons-outlined em-pointer"
          title="reset-all-filters">filter_alt_off</span>
                    ...
                    <div class="filter-container em-w-100 em-mb-16" data-filteruid="<?= $filter['uid'] ?>">
                        <div class="filter-header em-w-100 em-flex-row em-flex-space-between em-mb-8">
							...
							<?php if (!$filter['default']) : ?>
                                <span class="material-icons-outlined em-pointer remove-filter"
                                      data-filteruid="<?= $filter['uid'] ?>">delete</span>
							<?php endif; ?>
                        </div>
						<?php
						switch ($filter['type']) {
							...
							case 'text':
								?>
                                <input type="text" id="filter-<?= $filter['id'] ?>" name="filter<?= $filter['id'] ?>"
                                       value="<?= $filter['value'] ?>" class="em-w-100"/>
								<?php
								break;
							case 'date':
								?>
                                <input type="date" id="filter-<?= $filter['id'] ?>" name="filter<?= $filter['id'] ?>"
                                       value="<?= $filter['value'] ?>" class="em-w-100"/>
								<?php
								break;
						}
            ...
            <select id="filters-selection" name="filters-selection" class="em-w-100">
				...
				<?php foreach ($filters as $filter): ?>
                    <option value="<?= $filter['id'] ?>"
                            data-values="<?= base64_encode(json_encode($filter['values'])); ?>"
                            data-type="<?= $filter['type'] ?>"><?= $filter['label'] ?></option>
				<?php endforeach; ?>
            </select>
        </div>
        <div class="actions em-mt-16">
            <button id="apply-filters"
                    class="em-primary-button"><?= JText::_('MOD_EMUNDUS_FILTERS_APPLY_FILTERS'); ?></button>
            <button id="add-filter"
                    class="em-secondary-button em-mt-16"><?= JText::_('MOD_EMUNDUS_FILTERS_ADD_FILTER'); ?></button>
        </div>
		<?php
	}
	else {
