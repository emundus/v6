<template>
  <div id="emundus-filters" class="em-w-100">
    <section id="filters-top-actions" class="em-mb-16">
      <span id="clear-filters" class="material-icons-outlined em-pointer hidden" @click="clearFilters"
            :alt="translate('MOD_EMUNDUS_FILTERS_CLEAR_FILTERS')">filter_list_off</span>
      <span id="save-filters" class="material-icons-outlined em-pointer hidden" @click="onClickSaveFilter"
      ...
      <div id="global-search-wrapper" style="position: relative;">
        <div id="global-search-values" ref="globalSearchValues"
          ...
          <input id="current-global-search" ref="globalSearchInput" class="em-border-radius-8"
                 v-model="currentGlobalSearch" type="text" @keyup.enter="onGlobalSearchChange('everywhere')"
        ...
        <ul id="select-scopes"
          ...
          <li v-for="option in globalSearchScopes" :key="option.value" @click="onGlobalSearchChange(option.value)"
              class="em-pointer">{{ currentGlobalSearch }} {{ translate('MOD_EMUNDUS_FILTERS_SCOPE_IN') }}
      ...
      <div id="save-filters-inputs-btns">
        <div id="save-filter-new-name"
          ...
          <input id="new-filter-name" ref="new-filter-name" type="text" class="em-flex-row" v-model="newFilterName"
                 :placeholder="translate('MOD_EMUNDUS_FILTERS_SAVE_FILTER_NAME')" minlength="2"
                 @keyup.enter="saveFilters" @focusout="onFocusOutNewFilter">
          <span id="save-new-filter" class="material-icons-outlined em-pointer"
                :class="{'em-pointer em-dark-blue-500-color': newFilterName.length > 1}"
                @click="saveFilters">done</span>
        </div>
        <div v-if="registeredFilters.length > 0" id="registered-filters-wrapper" class="em-mt-8">
          ...
          <div class="em-flex-row em-flex-space-between">
            ...
            <span v-if="selectedRegisteredFilter > 0" class="material-icons-outlined em-red-500-color em-pointer"
    ...
    <section id="applied-filters">
      <div v-for="appliedFilter in appliedFilters" :key="appliedFilter.uid">
        ...
        <DefaultFilter v-else :filter="appliedFilter" :module-id="moduleId" class="em-w-100"
    ...
    <section id="filters-bottom-actions">
      <button id="em-add-filter" class="em-secondary-button em-white-bg em-mt-16"
              @click="openFilterOptions = !openFilterOptions">{{ translate('MOD_EMUNDUS_FILTERS_ADD_FILTER') }}
      </button>
      <button id="em-apply-filters" class="em-primary-button em-mt-16 hidden" @click="applyFilters">
