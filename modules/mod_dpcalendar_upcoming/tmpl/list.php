...
// Load the stylesheet
JHtml::_('stylesheet', 'mod_dpcalendar_upcoming/list.min.css', ['relative' => true]);

// The root container
$root = new Container('dp-module-upcoming-list-' . $module->id, array('root'), array('ccl-prefix' => 'dp-module-upcoming-list-'));
$root->addClass('dp-module-upcoming-root', true);

// The events container
$c = $root->addChild(new ListContainer('events', ListContainer::UNORDERED));
...
foreach ($events as $index => $event) {
	...
	if ($grouping) {
		...
		if ($groupHeading != $lastHeading) {
			...
			$c->addChild(new ListItem('heading-' . ($index + 1), array('heading')))->setContent($groupHeading);
		}
	}

	// The event container
	$ec = $c->addListItem(new ListItem($event->id, array('container')));
