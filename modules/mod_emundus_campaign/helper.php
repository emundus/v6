...
class modEmundusCampaignHelper {
    ...
    public function getCurrent($condition, $teachingUnityDates = null) {
        ...
        if ($teachingUnityDates) {
            $query->select('ca.*, pr.apply_online, pr.code,pr.label as programme,pr.color as tag_color, pr.link, tu.date_start as formation_start, tu.date_end as formation_end, pr.programmes as prog_type, pr.id as p_id, pr.notes,ca.is_limited')
            ...
            $query  = $db->getQuery(true);
            $query->select('ca.*, pr.apply_online, pr.code,pr.label as programme,pr.color as tag_color, pr.link, pr.programmes as prog_type, pr.id as p_id, pr.notes');
    ...
    public function getPast($condition, $teachingUnityDates = null) {
        ...
        if ($teachingUnityDates) {
            $query
                ->select('ca.*, pr.apply_online, pr.code,pr.label as programme,pr.color as tag_color, pr.link, tu.date_start as formation_start, tu.date_end as formation_end, pr.programmes as prog_type, pr.id as p_id, pr.notes,ca.is_limited')
                ->from($db->qn('#__emundus_setup_campaigns', 'ca'))
                ->leftJoin($db->qn('#__emundus_setup_programmes', 'pr') . ' ON ' . $db->qn('pr.code') . ' = ' . $db->qn('ca.training'))
                ->leftJoin($db->qn('#__emundus_setup_teaching_unity', 'tu') . ' ON ' . $db->qn('tu.code') . ' = ' . $db->qn('ca.training').' AND '.$db->quoteName('ca.year').' = '.$db->quoteName('tu.schoolyear'))
            ...
            $query
                ->select('ca.*, pr.apply_online, pr.code,pr.label as programme,pr.color as tag_color, pr.link,pr.programmes as prog_type')
    ...
    public function getFutur($condition, $teachingUnityDates = null) {
        ...
        if ($teachingUnityDates) {
            ...
            $query
                ->select('ca.*, pr.apply_online, pr.link,pr.label as programme,pr.color as tag_color,pr.programmes as prog_type')
                ->from('#__emundus_setup_campaigns as ca,#__emundus_setup_programmes as pr')
    ...
    public function getProgram($condition, $teachingUnityDates = null) {
        ...
        if ($teachingUnityDates) {
            $query
                ->select('ca.*, pr.apply_online, pr.code,pr.label as programme,pr.color as tag_color, pr.link, tu.date_start as formation_start, tu.date_end as formation_end, pr.notes as desc,ca.is_limited,pr.programmes as prog_type')
                ->from($db->qn('#__emundus_setup_campaigns', 'ca'))
                ->leftJoin($db->qn('#__emundus_setup_programmes', 'pr') . ' ON ' . $db->qn('pr.code') . ' = ' . $db->qn('ca.training'))
                ->leftJoin($db->qn('#__emundus_setup_teaching_unity', 'tu') . ' ON ' . $db->qn('tu.code') . ' = ' . $db->qn('ca.training').' AND '.$db->quoteName('ca.year').' = '.$db->quoteName('tu.schoolyear'))
            ...
            $query
                ->select('ca.*, pr.apply_online, pr.code,pr.label as programme,pr.color as tag_color, pr.link, pr.notes')
                ->from('#__emundus_setup_campaigns as ca, #__emundus_setup_programmes as pr')
                ->where('ca.training = pr.code AND ca.published=1 '.$condition);
        }
