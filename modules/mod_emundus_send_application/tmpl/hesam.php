		...
		<?php if ($print) : ?>
            <div class="col-md-12 em-print-button">
                <a id="print" class="btn btn-info btn-xs"
                   href="index.php?option=com_emundus&task=pdf&fnum=<?= $user->fnum; ?>" title="Print" target="_blank"
                   title="<?= JText::_('MOD_EMUNDUS_SEND_APPLICATION_PRINT_APPLICATION_FILE'); ?>"><i
                            class="icon-print"></i> <?= JText::_('MOD_EMUNDUS_SEND_APPLICATION_PRINT_APPLICATION_FILE'); ?>
                </a>
            </div>
		<?php endif; ?>
        <div class="suivant w-inline-block">
            <a class="entrer-en-contact" href="<?= JRoute::_(JURI::base() . $confirm_form_url); ?>"
               title="<?= JText::_('MOD_EMUNDUS_SEND_APPLICATION_SEND_APPLICATION_FILE'); ?>"><?= JText::_('MOD_EMUNDUS_SEND_APPLICATION_SEND_APPLICATION_FILE'); ?></a>
        </div>
	<?php elseif (modemundusSendApplicationHelper::getSearchEngineId($user->fnum) && $print) : ?>
        <div class="col-md-12 em-print-button">
            <a id="print" class="btn btn-info btn-xs"
               href="<?= JRoute::_(JURI::base() . 'consultez-les-offres/details/299/' . modemundusSendApplicationHelper::getSearchEngineId($application->fnum)) . '?format=pdf'; ?>"
