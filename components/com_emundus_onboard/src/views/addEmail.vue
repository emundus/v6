        ...
        <div class="sous-container last-container" v-if="email == ''">
          ...
          <div v-if="triggered">
            <div class="form-group">
              ...
              >
              <select
                v-model="trigger.action_status"
                class="dropdown-toggle w-select"
                :class="{ 'is-invalid': errors.trigger.action_status }"
              >
                <option value="to_current_user">
                  {{ translations.TheCandidate }}
                </option>
            ...
            <div class="form-group">
              ...
              <select
                ...
                class="dropdown-toggle w-select"
                :class="{ 'is-invalid': errors.trigger.status }"
              >
                <option
            ...
            <div class="form-group">
              ...
              <select
                ...
                class="dropdown-toggle w-select"
                :class="{ 'is-invalid': errors.trigger.target }"
              >
            ...
            <div
              class="form-group"
              v-if="trigger.target == 0"
              style="align-items: baseline"
            >
              <label
                >{{ translations.ChooseUsers
                }}<span style="color: #e5283b">*</span> :</label
              >
              <div class="wrap">
                <div class="search">
                  <input
                    ...
                    :placeholder="Search"
                    v-model="searchTerm"
                    @keyup="searchUserByTerm"
                  />
                  <button
                    type="button"
                    class="searchButton"
                    @click="searchUserByTerm"
                  >
              ...
              <div
                class="users-block"
                :class="{ 'is-invalid': errors.trigger.selectedUsers }"
              >
                <div
...
export default {
  ...
  methods: {
    ...
    getStatus() {
      axios
        .get(
          "index.php?option=com_emundus_onboard&controller=email&task=getstatus"
        )
        .then((response) => {
    ...
    getUsers() {
      axios
        .get(
          "index.php?option=com_emundus_onboard&controller=program&task=getuserswithoutapplicants"
        )
        .then((response) => {
    ...
    searchUserByTerm() {
      axios
        .get("index.php?option=com_emundus_onboard&controller=program&task=searchuserbytermwithoutapplicants&term=" + this.searchTerm)
        .then((response) => {
          this.users = response.data.data;
        });
    },
    ...
    submit() {
      ...
      if (this.email !== "") {
        axios({
          ...
          .then(() => {
            this.redirectJRoute(
              "index.php?option=com_emundus_onboard&view=email"
            );
          })
          .catch((error) => {
            console.log(error);
    ...
    tip: function () {
      this.show(
        "foo-velocity",
        Joomla.JText._("COM_EMUNDUS_ONBOARD_VARIABLESTIP") +
  ...
  created() {
    ...
    axios
      .get(
        "index.php?option=com_emundus_onboard&controller=email&task=getemailcategories"
      )
      .then((rep) => {
          ...
          axios
            ...
            .then((resp) => {
              ...
              if (resp.data.data.letter_attachment) {
                this.selectedLetterAttachments =
                  resp.data.data.letter_attachment;
              }

              // get attached candidate attachments
              if (resp.data.data.candidate_attachment) {
                this.selectedCandidateAttachments =
                  resp.data.data.candidate_attachment;
              }
              ...
              if (
                resp.data.data.receivers !== null &&
                resp.data.data.receivers !== undefined &&
                resp.data.data.receivers !== ""
              ) {
                let receiver_cc = [];
                let receiver_bcc = [];
                ...
                for (let index = 0; index < receivers.length; index++) {
                  ...
                  if (
                    receivers[index].type === "receiver_cc_email" ||
                    receivers[index].type === "receiver_cc_fabrik"
                  ) {
