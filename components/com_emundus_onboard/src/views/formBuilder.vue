<template>
  <div class="container-fluid">
    ...
    <div v-if="indexHighlight != -1">
      <ModalAffectCampaign :prid="prid" :testing="testing" />
      <ModalTestingForm
    ...
    <div class="row" v-if="indexHighlight != -1">
      <div
        class="sidebar-formbuilder"
        :style="actions_menu ? 'width: 250px' : ''"
      >
        <transition name="move-right">
          <div class="actions-menu menu-block">
            ...
            <a
              class="d-flex back-button-action pointer"
            ...
            <div>
              <div class="action-links">
                <a
                  class="d-flex action-link"
                  style="padding-top: 2em"
                  @click="$modal.show('modalMenu')"
                  :title="translations.addMenu"
                >
                  <em class="add-page-icon"></em>
                  <label class="action-label col-md-offset-1 col-sm-offset-1">{{
                ...
                <a
                  class="d-flex action-link"
                  :class="{ 'disable-element': elementDisabled }"
                  @click="showElements"
                  :title="translations.addItem"
                >
                  <em class="add-element-icon"></em>
                  <label
                    class="action-label col-md-offset-1 col-sm-offset-1"
                    :class="[
                      { 'disable-element': elementDisabled },
                      addingElement ? 'down-arrow' : 'right-arrow',
                    ]"
                    >{{ translations.addItem }}</label
                  >
                </a>
                <transition :name="'slide-right'" type="transition">
                  <div class="plugins-list" v-if="addingElement">
                    <a
                      class="d-flex col-md-offset-1 back-button-action pointer"
                      style="padding: 0 15px"
                      @click="addingElement = !addingElement"
                      :title="Back"
                    >
                    ...
                    >
                      <div
                        ...
                        :key="'plugin_' + index"
                        :id="'plugin_' + plugin.value"
                        @dblclick="addingNewElementByDblClick(plugin.value)"
                        :title="plugin.name"
                      >
      ...
      <div
        :class="
          actions_menu
            ? 'col-md-8 col-md-offset-4 col-sm-9 col-sm-offset-3'
            : ''
        "
        class="menu-block"
      >
        <div
          class="heading-block"
          :class="addingElement || actions_menu ? 'col-md-6' : 'col-md-8'"
        >
          <div class="d-flex form-title-block" v-show="!updateFormLabel">
            <h2
              class="form-title"
              @click="enableUpdatingForm"
          ...
          <div
            style="width: max-content; margin-left: 20px"
            v-show="updateFormLabel"
          >
            <div class="input-can-translate">
              <input
                v-model="profileLabel"
                class="form__input field-general w-input"
                style="width: 400px"
                @keyup.enter="updateLabelForm()"
                :id="'update_label_form_' + prid"
              />
        ...
        <div v-if="menuHighlight === 0" class="form-builder">
          <div
            class="form-viewer-builder"
            :class="[
              addingElement || actions_menu
                ? 'col-sm-offset-5 col-md-offset-4 col-lg-offset-1 col-sm-7 col-md-6'
                : 'col-md-8',
              optionsModal ? 'col-sm-5 col-md-6' : 'col-md-8',
            ]"
          >
            <Builder
        ...
        <ul
          class="col-md-3 sticky-form-pages"
          :class="[
            addingElement || (actions_menu && formList.length > 0)
              ? 'ml-10px col-sm-offset-5 col-sm-7'
              : '',
            optionsModal ? 'col-sm-5' : '',
            formList.length === 0 ? 'col-sm-offset-5 col-sm-7' : '',
          ]"
          style="margin-top: 0"
          v-if="formObjectArray"
        >
          <div class="d-flex justify-content-between mb-1">
            <h3 class="mb-0" style="padding: 0">{{ translations.FormPage }}</h3>
            <label class="saving-at"
              >{{ translations.Savingat }} {{ lastUpdate
              }}<em class="fas fa-sync ml-10px"></em
            ></label>
          </div>
          <div class="form-pages">
            <h4 class="ml-10px form-title" style="margin-bottom: 0; padding: 0">
              <img
                src="/images/emundus/menus/form.png"
                class="mr-1"
                :alt="translations.Form"
              />{{ translations.Form }}
            </h4>
            <draggable
              handle=".handle"
            ...
            >
              ...
              >
                <span
                  class="icon-handle"
                  :style="
                    grab && indexGrab == index ? 'opacity: 1' : 'opacity: 0'
                  "
                >
                ...
                <a
                  @click="
                    changeGroup(index, value.rgt);
                    menuHighlight = 0;
                  "
                  class="MenuFormItem"
                  :title="value.label"
                  :class="
                    indexHighlight == index && menuHighlight === 0
            ...
            <button
              ...
              @click="
                $modal.show('modalMenu');
                optionsModal = true;
              "
              style="margin-left: 30px"
              :title="translations.addMenuAction"
            >
              {{ translations.addMenuAction }}
            </button>
          </div>
          <div class="form-pages">
            <h4
              class="ml-10px form-title"
              style="margin-bottom: 10px; padding: 0"
            >
              <em
                class="far fa-folder-open mr-1"
                :alt="translations.Documents"
              ></em
            ...
            >
              <li
                ...
                :key="index"
                class="MenuForm"
                @mouseover="enableGrabDocuments(index)"
                @mouseleave="disableGrabDocuments()"
                v-if="doc.displayed == 1"
              >
                <span
                  class="icon-handle"
                  :style="
                    grabDocs && indexGrabDocuments == index
                      ? 'opacity: 1'
                      : 'opacity: 0'
                  "
                >
                ...
                <a class="MenuFormItem" :title="doc.label">
                  {{ doc.label
                  }}<span v-if="doc.mandatory == 1" style="color: red">*</span>
                </a>
                <a
                  class="cta-block pointer"
                  @click="removeDocument(index, doc.id)"
                  :style="
                    grabDocs && indexGrabDocuments == index
                      ? 'opacity: 1'
                      : 'opacity: 0'
                ...
                <a
                  @click="
                    currentDoc = doc.docid;
                    $modal.show('modalAddDocuments');
                    optionsModal = true;
                  "
                  :title="translations.Edit"
                  class="cta-block pointer"
            ...
            <button
              ...
              "
              style="margin-left: 30px"
              :title="translations.AddNewDocument"
            >
          ...
          >
            ...
            >
              <img
                src="/images/emundus/menus/confirmation.png"
                class="mr-1"
                :alt="translations.SubmitPage"
              />{{ translations.SubmitPage }}
            </h4>
            <li
              v-for="(value, index) in submittionPages"
              :key="index"
              class="MenuForm"
            >
              <a
                ...
                "
                :class="
                  indexHighlight == index && menuHighlight === 1
                    ? 'MenuFormItem_current'
                    : ''
                "
              >
          ...
          <div class="d-flex">
            ...
            <button
              class="bouton-sauvergarder-et-continuer mt-1"
              @click="exitForm"
              style="margin-left: 10px"
              :title="translations.Validate"
            >
              {{ translations.ExitFormbuilder }}
            </button>
          </div>
        </ul>
      </div>
  ...
  data() {
    return {
      ...
      translations: {
        ...
        addMenuAction: Joomla.JText._(
          "COM_EMUNDUS_ONBOARD_BUILDER_ADDMENU_ACTION"
        ),
        ...
        updateSuccess: Joomla.JText._(
          "COM_EMUNDUS_ONBOARD_BUILDER_UPDATESUCESS"
        ),
        updateFailed: Joomla.JText._(
          "COM_EMUNDUS_ONBOARD_BUILDER_UPDATEFAILED"
        ),
        ExitFormbuilder: Joomla.JText._(
          "COM_EMUNDUS_ONBOARD_BUILDER_EXIT_FORMBUILDER"
        ),
        BuildYourForm: Joomla.JText._(
          "COM_EMUNDUS_ONBOARD_BUILDER_BUILD_YOUR_FORM"
        ),
        SubmittionPage: Joomla.JText._(
          "COM_EMUNDUS_ONBOARD_BUILDER_SUBMITTION_PAGE"
        ),
      },
    };
  },
  ...
  methods: {
    ...
    async createElement(gid, plugin, order) {
      ...
      if (!_.isEmpty(list[this.indexHighlight].object.Groups)) {
        ...
        if (plugin == "emundus_fileupload") {
          ...
          const createSimpleElement = await formBuilderService.createElement(
            parseInt(gid),
            plugin
          );

          if (createSimpleElement != false) {
            const getCreatedElement = await formBuilderService.getElement(
              gid,
              createSimpleElement.scalar
            );
            ...
            const updateElementParams =
              await formBuilderService.updateElementParams(
    ...
    async createElementEMundusFileUpload(params, gid, plugin, order) {
      ...
      if (this.updateDocument != false) {
        ...
        const createElement = await formBuilderService.createElement(
          gid,
          plugin,
          updateDocument.data
        );
    ...
    menuHighlightCustumisation(response, gid, order) {
      if (this.menuHighlight === 0) {
        this.$set(
          this.formObjectArray[this.indexHighlight].object.Groups[
            "group_" + gid
          ],
          "elements[element" + response.data.id + "]",
          response.data
        );
        this.formObjectArray[this.indexHighlight].object.Groups[
          "group_" + gid
        ].elts.splice(order, 0, response.data);
        this.$refs.builder.updateOrder(
          gid,
          this.formObjectArray[this.indexHighlight].object.Groups[
            "group_" + gid
          ].elts
        );
        this.$refs.builder.$refs.builder_viewer.keyElements[
          "element" + response.data.id
        ] = 0;
        this.$refs.builder.$refs.builder_viewer.enableActionBar(
          response.data.id
        );
        this.$refs.builder.$refs.builder_viewer.enableLabelInput(
          response.data.id
        );
      } else {
        this.$set(
          this.submittionPages[this.indexHighlight].object.Groups[
            "group_" + gid
          ],
          "elements[element" + response.data.id + "]",
          response.data
        ...
        this.$refs.builder_submit.updateOrder(
          gid,
          this.submittionPages[this.indexHighlight].object.Groups[
            "group_" + gid
          ].elts
        );
        this.$refs.builder_submit.$refs.builder_viewer.keyElements[
          "element" + response.data.id
        ] = 0;
        this.$refs.builder_submit.$refs.builder_viewer.enableActionBar(
          response.data.id
        );
        this.$refs.builder_submit.$refs.builder_viewer.enableLabelInput(
          response.data.id
        );
      }
    },
    addingNewElement: function (evt) {
      ...
      if (
        typeof document.getElementsByClassName("no-elements-tip")[0] !=
        "undefined"
      ) {
        document.getElementsByClassName("no-elements-tip")[0].style.background =
          "#e4e4e9";
        document.getElementsByClassName("no-elements-tip")[0].style.border =
          "2px dashed #c3c3ce";
        document.getElementsByClassName("no-elements-tip")[0].innerHTML =
          Joomla.JText._("COM_EMUNDUS_ONBOARD_NO_ELEMENTS_TIPS");
      }
      let plugin = evt.clone.id.split(/_(.+)/)[1];
      let gid =
        evt.to.parentElement.parentElement.parentElement.id.split("_")[1];
      if (typeof gid != "undefined") {
        this.createElement(gid, plugin, evt.newIndex);
      }
    },
    addingNewElementByDblClick: _.debounce(
      function (plugin) {
        let gid = Object.keys(
          this.formObjectArray[this.indexHighlight].object.Groups
        )[
          Object.keys(this.formObjectArray[this.indexHighlight].object.Groups)
            .length - 1
        ].split("_")[1];
        let index =
          this.formObjectArray[this.indexHighlight].object.Groups[
    ...
    async createGroup() {
      ...
      /*try {
          const createGroup = await formBuilderService.createGroup(param).then(response => {
              ...
              data: qs.stringify({
                toJTEXT: response.group_tag
              })
            }).then((traductions) => {
              response.data.label.fr = traductions.data.fr;
              response.data.label.en = traductions.data.en;
              this.loading = false;
              this.pushGroup(response);
            });
      ...
      axios({
        ...
        axios({
          ...
          console.log(
            "Good day my Guys this the best ways to performs your need"
          );
          console.log(result.data.group_showLegend);
          axios({
            method: "post",
            url: "index.php?option=com_emundus_onboard&controller=formbuilder&task=getalltranslations",
            headers: {
    ...
    async updateLabelForm() {
      try {
        await formService
          .updateFormLabel(this.prid, this.profileLabel)
          .then(() => {
            this.show(
              "foo-velocity",
              "success",
      ...
      if (this.menuHighlight === 0) {
        this.formObjectArray.forEach((form, index) => {
          if (form.object.id == group.formid) {
            this.formObjectArray[index]["object"]["Groups"][
        ...
        this.$refs.builder.$refs.builder_viewer.openGroup[
          group.group_id
        ] = true;
      } else {
        this.submittionPages.forEach((form, index) => {
          if (form.object.id == group.formid) {
            this.submittionPages[index]["object"]["Groups"][
              "group_" + group.group_id
            ] = {
              elements: {},
              elts: [],
        ...
        this.$refs.builder_submit.$refs.builder_viewer.openGroup[
          group.group_id
        ] = true;
      }
    ...
    async pushMenu(menu) {
      ...
      try {
        await formService.pushMenu(menu.id).then((response) => {
    ...
    removeGroup(group_id, form_id) {
      this.formObjectArray.forEach((form, index) => {
        if (form.object.id == form_id) {
          delete this.formObjectArray[index]["object"]["Groups"][
            "group_" + group_id
          ];
        }
      });
    ...
    async getElement(element, gid) {
      try {
        await formBuilderService.getElement(gid, element).then((response) => {
          this.formObjectArray[this.indexHighlight].object.Groups[
            "group_" + gid
          ].elements["element" + response.data.id] = response.data;
          this.formObjectArray[this.indexHighlight].object.Groups[
            "group_" + gid
          ].elts.push(response.data);
          this.builderKey += 1;
        });
      } catch (error) {
        console.log(error);
    ...
    tip() {
      this.showTip(
        "foo-velocity",
        Joomla.JText._("COM_EMUNDUS_ONBOARD_UPDATEFORMTIP") +
          "<br/>" +
          Joomla.JText._("COM_EMUNDUS_ONBOARD_UPDATEFORMTIP1") +
          "<br/>" +
          Joomla.JText._("COM_EMUNDUS_ONBOARD_UPDATEFORMTIP2"),
    ...
    show(group, type = "", text = "", title = "Information") {
      ...
      this.lastUpdate =
        ...
        (new Date().getMinutes() < 10 ? "0" : "") +
        new Date().getMinutes();
      document.getElementsByClassName("fa-sync")[0].style.transform =
        "rotate(360deg)";
      setTimeout(() => {
    ...
    async getDataObject() {
      ...
      this.elementDisabled = _.isEmpty(
        this.formObjectArray[this.indexHighlight].object.Groups
      );
    ...
    async getDataObjectSingle(index) {
      if (this.formList.length > 0) {
        let ellink = this.formList[index].link.replace(
          "fabrik",
          "emundus_onboard"
        );
        await axios
    ...
    async getFilesByForm() {
      try {
        await formService.getFilesByForm(this.prid).then((response) => {
    ...
    async getSubmittionPage() {
      try {
        await formService.getSubmissionPage(this.prid).then((response) => {
          this.submittionPages.push({
            object: response.responseAfterVueJsonClean,
            rgt: response.rgt,
            link: response.link,
          });
        });
    ...
    async getDocuments() {
      try {
        const retrievedDocumentList = await formService.getDocuments(this.prid);
        this.documentsList = retrievedDocumentList.data;
      } catch (error) {
        console.log(error);
      }
    },

    async removeDocument(index, did) {
      try {
        await formService.removeDocumentFromProfile(did).then(() => {
     ...
     */
    async getProfileLabel(profile_id) {
      try {
        await formService
          .getProfilelabelByProfileId(profile_id)
          .then((response) => {
            this.profileLabel = response.data.label;
          });
      } catch (error) {
        console.log(error);
      }
    ...
    async exitForm() {
      try {
        await settingsService
          .redirectjRoute("index.php?option=com_emundus_onboard&view=form")
          .then((response) => {
            window.location.href = window.location.pathname + response.data;
          });
      } catch (error) {
        console.log(error);
      }
      ...
      if (this.formList.length > 0) {
        if (this.cid != 0) {
          try {
            await settingsService
              .redirectjRoute(
                "index.php?option=com_emundus_onboard&view=form&layout=addnextcampaign&cid=" +
                  this.cid +
                  "&index=4"
              )
              .then((response) => {
          ...
          try {
            await formService
              .getAssociatedCampaign(this.prid)
              .then((response) => {
          ...
          .then((response) => {
            ...
            if (Object.keys(this.campaignsAffected).length > 1) {
              ...
              if (this.campaignsAffected[0].files.length > 0) {
                ...
                formBuilderService
                  .createTestingFile(this.campaignsAffected[0].id)
                  .then((response) => {
                    window.open(
                      "/index.php?option=com_emundus&task=openfile&fnum=" +
                        response.data.fnum +
                        "&redirect=1==&Itemid=1079#em-panel"
                    );
                  });
              }
            } else {
              ...
              formBuilderService.getAssociatedCampaign(this.prid).then(() => {
                this.$modal.show("modalAffectCampaign");
              });
            }
          });
    ...
    changeGroup(index) {
      ...
      document.cookie =
        "page_" + this.prid + "=" + index + "; expires=Session; path=/";
    },
    ...
    // Draggable documents
    reorderingDocuments: async function () {
      this.documentsList.forEach((doc, index) => {
        doc.ordering = index;
      });
      try {
        await formService.reorderDocument(this.documentsList);
      } catch (error) {
        console.log(error);
      }
    },
    ...
    reorderItems() {
      this.formList.forEach((item) => {
        try {
          formBuilderService.reorderMenuItems(item.rgt, item.link);
        } catch (error) {
          console.log(error);
        }
      });
