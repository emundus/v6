...
export default {
    ...
    async getTestingParams(pid) {
        try {
            const response = await client().get("index.php?option=com_emundus_onboard&controller=formbuilder&task=gettestingparams", { params: { prid: gid } });
            return response.data;
        } catch (e) {
            return false;
        }
    ...
    async createTestingFile(campaignId) {
        try {
            const response = await client().post("index.php?option=com_emundus_onboard&controller=formbuilder&task=createtestingfile", { cid: campaignId });
            return response.data;
        } catch (e) {
            return false;
        }
    ...
    async reorderMenuItems(rgt, link) {
        try {
            const response = await client().post("index.php?option=com_emundus_onboard&controller=formbuilder&task=reordermenu", { rgt: rgt, link: link })
