...
class SimpleXLSX
{
    ...
    function worksheet($worksheet_id)
    {
        if (isset($this->sheets[$worksheet_id])) {
            ...
            $this->error('Worksheet ' . $worksheet_id . ' not found. Try $xlsx->rows(' . implode(
                ') or $xlsx->rows(',
...
// echo 'workbook relations<br />';
                            ...
                            foreach ($workbookRelations->Relationship as $workbookRelation) {
                                ...
                                if ($workbookRelation['Type'] == self::SCHEMA_REL_WORKSHEET) { // Sheets
// echo 'sheet<br />';
...
// echo '<pre>'.htmlspecialchars( print_r( $sheet, true ) ).'</pre>';
                                ...
                                } elseif ($workbookRelation['Type'] == self::SCHEMA_REL_SHAREDSTRINGS && $this->entryExists($path)) { // 0.6.6
// echo 'sharedstrings<br />';
                                ...
                                } elseif ($workbookRelation['Type'] == self::SCHEMA_REL_STYLES) {
