 ...
 */
var bootbox=window.bootbox||(function(document,$){var _animate=!0,_backdrop='static',_defaultHref='javascript:;',_classes='dropfiles',_icons={},that={};that.setIcons=function(icons){_icons=icons;if(typeof _icons!=='object'||_icons==null){_icons={}}};that.alert=function(){var str="",label=_translate('OK'),cb=null;switch(arguments.length){case 1:str=arguments[0];break;case 2:str=arguments[0];if(typeof arguments[1]=='function'){cb=arguments[1]}else{label=arguments[1]}
    ...
    return that.dialog(str,{"label":label,"icon":_icons.OK,"callback":cb},{"onEscape":cb||!0})};that.confirm=function(){var str="",labelCancel=_translate('CANCEL'),labelOk=_translate('CONFIRM'),cb=null;switch(arguments.length){case 1:str=arguments[0];break;case 2:str=arguments[0];if(typeof arguments[1]=='function'){cb=arguments[1]}else{labelCancel=arguments[1]}
    ...
    var header=str;var form=$("<form></form>");form.append("<input autocomplete=off type=text value='"+defaultVal+"' />");var cancelCallback=function(){if(typeof cb==='function'){cb(null)}};var confirmCallback=function(){if(typeof cb==='function'){cb(form.find("input[type=text]").val())}};var div=that.dialog(form,[{"label":labelCancel,"icon":_icons.CANCEL,"callback":cancelCallback},{"label":labelOk,"icon":_icons.CONFIRM,"callback":confirmCallback}],{"header":header,"show":!1,"onEscape":cancelCallback});div.on("shown",function(){form.find("input[type=text]").focus();form.on("submit",function(e){e.preventDefault();div.find(".btn-primary").click()})});div.modal("show");div.fadeIn();return div};that.dialog=function(str,handlers,options){var buttons="",callbacks=[],options=options||{};if(handlers==null){handlers=[]}else if(typeof handlers.length=='undefined'){handlers=[handlers]}
    ...
    div.find(".modal-body").html(str);div.on('hidden',function(){div.remove()});div.on('keyup.dismiss.modal',function(e){if(e.which==27&&options.onEscape){if(typeof options.onEscape==='function'){options.onEscape()}
        ...
        if(hideModal!==!1){div.modal("hide");$('.modal-backdrop').remove()}});if($("body").hasClass("admin")){$("#mybootstrap").append(div)}else{$("body").append(div)}
    div.modal({backdrop:(typeof options.backdrop==='undefined')?_backdrop:options.backdrop,keyboard:!1,show:!1});div.on("show",function(e){$(document).off("focusin.modal")});if(typeof options.show==='undefined'||options.show===!0){div.modal("show");div.addClass('in')}
    return div};that.modal=function(){var str;var label;var options;var defaultOptions={"onEscape":null,"keyboard":!0,"backdrop":_backdrop};switch(arguments.length){case 1:str=arguments[0];break;case 2:str=arguments[0];if(typeof arguments[1]=='object'){options=arguments[1]}else{label=arguments[1]}
    ...
    return that.dialog(str,[],options)};that.hideAll=function(){$(".bootbox").modal("hide")};that.animate=function(animate){_animate=animate};that.backdrop=function(backdrop){_backdrop=backdrop};that.classes=function(classes){_classes=classes};function _translate(str){return Joomla.JText._('COM_DROPFILES_JS_'+str,str)}
    return that}(document,window.jQuery));window.bootbox=bootbox