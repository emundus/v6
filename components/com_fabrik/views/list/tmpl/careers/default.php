...
<form class="fabrikForm form-search" action="<?php echo $this->table->action;?>" method="post" id="<?php echo $this->formid;?>" name="fabrikList" data-id="<?= $form_id; ?>">
    ...
    <input type="hidden" id="menu_id" value="<?= $menu_id->id; ?>">
        ...
        <div id="careers" class="section-normal">
            <div class="w-container">
                <h3 class="h3-lascala center dark">Alumni, what are they doing now?</h3>
                <?php
        ...
        foreach ($this->rows as $groupedby => $group) {
            ...
            for($j=0; $j < count($classArray); $j++) {
                for ($h = 0; $h < count($classArray); $h++) {
                    ...
                    if (end($element) == 'em-country') {
                        $country = $element[0];
                    }
                    if (end($element) == 'em-mobility') {
                        $mobility = $element[0];
                    }
                    if (end($element) == 'em-position') {
                        $position = $element[0];
                    }
            ...
            for($i=1; $i <= count($group); $i++) {
                ...
                //Pair
                ?>
                    <div class="careers-blocs">
                        <div class="careers-image">
                            <img src="<?= JUri::base() .$group[$i]->data->$filename_raw; ?>"/>
                        </div>
                        <div class="careers-text">
                            ...
                            <p><?= $group[$i]->data->$country; ?></p>
                            <p><?= $group[$i]->data->$mobility; ?></p>
                            <p><strong>Current postion/job tittle:</strong> <?= $group[$i]->data->$position; ?></p>
                        </div>
                    </div>    
            <?php } 
        } ?>
