...
class DPCalendarModelEvents extends JModelList
{
	...
	public function getItems ()
	{
		...
		foreach ($items as $key => $item)
		{
			...
			if (! JFactory::getUser()->authorise('core.admin', 'com_dpcalendar') && ! in_array($item->access_content, JFactory::getUser()->getAuthorisedViewLevels()))
			{
	...
	protected function getListQuery ()
	{
		...
		if (! empty($searchString))
		{
			...
			else
			{
				...
				{
					$query->join('LEFT', '#__dpfields_fields_values AS dpf ON dpf.item_id = a.id and dpf.context = ' . $db->q('com_dpcalendar.event'));
					$searchColumns[] = 'dpf.value';
				}
	...
	{
		// On some environments mysql throws the following error
		// mysqli_fetch_object(): Couldn't fetch mysqli_result
		$t = new DPCalendarTableEvent();
