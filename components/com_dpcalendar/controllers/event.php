...
class DPCalendarControllerEvent extends JControllerForm
{
	...
	public function move ()
	{
		...
		$success = false;
		$model = $this->getModel('form', 'DPCalendarModel', array(
		...
		else
		{
			...
			{
				$id = $data['id'];
				// If the id is numeric, then we are editing an event in
				// advanced
				// cache mode
		...
		if ($success)
		{
			DPCalendarHelper::sendMessage(JText::_('JLIB_APPLICATION_SAVE_SUCCESS'), false,
					array(
							'url' => DPCalendarHelperRoute::getEventRoute($data['id'], $data['catid'])
					));
		}
		else
		{
	...
	public function save ($key = null, $urlVar = 'e_id')
	{
		...
		if ($result)
		{
			if ($this->getTask() == 'save')
			{
				...
				$return = $this->getReturnPage();
				if ($return == JURI::base() &&
						 ($calendar->external || JFactory::getUser()->authorise('core.edit.state', 'com_dpcalendar.category.' . $data['catid'])))
				{
			...
			{
				$return = $this->getReturnPage();
				if ($calendar->external || JFactory::getUser()->authorise('core.edit.state', 'com_dpcalendar.category.' . $data['catid']))
				{
	...
	public function invite ()
	{
		...
		$data = $this->input->post->get('jform', array(), 'array');
		$this->getModel()->invite($data['event_id'], isset($data['users']) ? $data['users'] : array(), isset($data['groups']) ? $data['groups'] : array());
	...
	public function overlapping ()
	{
		...
		$startDate = DPCalendarHelper::getDateFromString($data['start_date'], $data['start_date_time'], $data['all_day'] == '1');
		$endDate = DPCalendarHelper::getDateFromString($data['end_date'], $data['end_date_time'], $data['all_day'] == '1');
		...
		$message = DPCalendarHelper::renderEvents($events,
