...
<!--          <div class="em-font-size-14 em-pointer em-p-8-12 em-hover-background-neutral-300">{{ translate('COM_EMUNDUS_ONBOARD_ATTACHMENT_STORAGE_GED_ALFRESCO_DELETE') }}</div>-->
    ...
    <div v-for="node in nodes">
      <div class="em-flex-row em-mb-16" :class="'em-level-' + node.level">
        ...
        <select class="em-ml-8 em-mr-8 em-clear-dropdown" v-else-if="node.level === 1">
        ...
        <v-popover :popoverArrowClass="'custom-popover-arrow'">
          ...
          <template slot="popover">
            ...
            <div class="em-font-size-14 em-pointer em-p-8-12 em-hover-background-neutral-300" @click="deleteNode(node.id)">{{ translate('COM_EMUNDUS_ONBOARD_ATTACHMENT_STORAGE_GED_ALFRESCO_DELETE') }}</div>
          </template>
        </v-popover>
      </div>
      ...
      <div v-for="children in node.childrens" class="em-flex-row" :class="'em-level-' + children.level">
        ...
        <v-popover :popoverArrowClass="'custom-popover-arrow'">
          ...
          <template slot="popover">
            ...
            <div class="em-font-size-14 em-pointer em-p-8-12 em-hover-background-neutral-300" @click="deleteNode(children.id)">{{ translate('COM_EMUNDUS_ONBOARD_ATTACHMENT_STORAGE_GED_ALFRESCO_DELETE') }}</div>
          </template>
        </v-popover>
      </div>
    </div>
