    ...
    <div v-for="translation in translations_rows" class="em-mb-32 em-neutral-100-box em-p-24">
      <div v-for="(field,index) in translation" class="em-mb-24">
        ...
        <div class="em-flex-space-between em-mt-16 em-grid-50 em-ml-24">
          ...
          <editor-quill v-if="field.field_type === 'editor'" :enable_variables="false" :text="field.lang_to" v-model="field.lang_to" :key="dynamicComponent" :id="'editor_'+field.reference_id" @input="debounce(() => {saveTranslation($event,field);}, 2000)();" />
        </div>
      </div>
    </div>
  </div>
