<template>
  <div class="em-w-100">
    <div class="button-container">
      <button v-for="(item, index) in Menus" :id="'Menu-'+index" name="BoutonMenu" :class="{ 'outlined-button': isOutlined }" @click="handleMenuButtonClick(index)">{{item.label}}</button>
    </div>
    <div v-if="indexMenu != null" class="submenu-div" v-for="(item, index1) in SubMenus[indexMenu]">
      <boutton class="outlined-button">{{SubMenus[indexMenu][index1].label}} </boutton>
      <div>
        <p>{{SubMenus[indexMenu][index1].options}}</p>
      </div>
    </div>
    <div class="em-page-loader" v-if="loading"></div>

    <aside id="logo-sidebar" class="fixed left-0 z-50 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
...
export default {
  ...
  methods: {
    ...
    handleMenuButtonClick(index) {
      let bool = this.isClicked[index]
      // set all buttons to false
      for (let key in this.isClicked) {
        this.isClicked[key] = false;
      }
      //toggle the clicked button
      this.isClicked[index] = !bool;
