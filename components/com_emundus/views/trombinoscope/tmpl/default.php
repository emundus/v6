    ...
    <div class="em-container-trombi">
        <div class="em-col-trombi">
            ...
            <?php foreach ($this->htmlLetters as $htmlLetter){ ?>
                ...
                <option value="<?= $htmlLetter['title']; ?>"><?= $htmlLetter['title']; ?></option>
...
<!-- <div class="panel-footer">
    <div class="row">
        <div class="col-md-3">
            <button onclick="window.location.hash='preview';" type="button" id="trombi_preview" class="btn btn-info"><?php echo JText::_('COM_EMUNDUS_TROMBI_PREVIEW')?>
                <span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span>
            </button>
           <button onclick="window.location.hash='trombi_download';" type="button" id="trombi_generate" class="btn btn-primary"><?php echo JText::_('COM_EMUNDUS_TROMBI_GENERATE')?>
                <span class="glyphicon glyphicon-file" aria-hidden="true"></span>
            </button>
        </div>
...
<script>
    $('.trombi_format').change(function() {
        var selected_format = this.value;

        var default_tmpl = '';
        //var trombi_tmpl = '<?php echo str_replace("\r\n", '', $this->trombi_tmpl);?>';
        //var badge_tmpl = '<?php echo str_replace("\r\n", '', $this->badge_tmpl);?>';

        var templ = <?php echo json_encode($this->templ) ?>;
    ...
    $('#trombi_preview').click(function (e) { 
        ...
        var data = $.ajax({
            ...
            async: false,
            dataType: "json",
            data: 'string_fnums='+string_fnums+'&grid='+selected_grid+'&margin='+selected_margin+'&template='+selected_tmpl+'&format='+format+'&generate='+string_generate,
