...
$fnum = $formModel->getElementData('jos_emundus_scholarship___fnum');
$mail_excelia = $formModel->getElementData('jos_emundus_scholarship___mail_excelia');
$is_excelia = $formModel->getElementData('jos_emundus_scholarship___is_excelia')[0];
$prog= '';

$int_progs = ['CPB', 'BFP', 'IEF'];

if (!empty($mail_excelia) && $is_excelia == 'oui') {
// We need to get every single group associated to a candidat intern
$query = 'SELECT p.Code_Programme FROM data_aurion_35676200 AS p
LEFT JOIN data_aurion_35676200 AS u on u.id_Programme = p.id_Programme
WHERE u.MailEcole LIKE ' . $db->quote($mail_excelia);
...
// We need to get every single group associated to a candidat intern
$query = 'SELECT DISTINCT(g.id) FROM #__emundus_setup_groups AS g WHERE g.concours IS NOT NULL';

try {

$db->setQuery($query);
$all_candidat_groups = $db->loadColumn();

} catch (Exception $e) {
var_dump($query).die();
}

if (!empty($prog) && in_array($prog,$int_progs)) {
// get Group IDs linked to Concours International
$query = 'SELECT DISTINCT(g.id) FROM #__emundus_setup_groups AS g WHERE g.concours = 2';
}
else {
// get Group IDs not linked to Concours France
$query = 'SELECT DISTINCT(g.id) FROM #__emundus_setup_groups AS g WHERE g.concours = 1';
}
...
if(!empty($all_candidat_groups)) {
// Delete assignements to groups, in case this isn't the first time this form is submitted.
$query = 'DELETE FROM #__emundus_group_assoc WHERE group_id IN ('.implode(',', $all_candidat_groups).') AND fnum LIKE '.$db->Quote($fnum);
try {
