...
<div>
	<div>
		<?php foreach($this->users as $user):?>
			<h4 title="<?php echo $user['fnum']?>"><?php echo $user['name']?> (<?php echo $user['label']?> - <?php echo $user['year']?>)</h4>
						...
						<tr class="em-actions-table-line">
							...
							<?php if($action['c'] == 1): ?>
								<td  id="c-check-<?php echo $action['id']?>" class="em-has-checkbox" >
									<input type="checkbox" class="em-modal-check c-check" name="c-check-<?php echo $action['id']?>" id="c-check-<?php echo $action['id']?>" <?php echo (@$this->actions_evaluators->{@$action['id']}->c == 1)?'checked':''; ?> /></td>
							<?php else:?>
								<td class="em-no no-action-c"></td>
							<?php endif?>
							<?php if($action['r'] == 1): ?>
								<td  id="r-check-<?php echo $action['id']?>" class="em-has-checkbox" ><input type="checkbox" class="em-modal-check r-check" name="r-check-<?php echo $action['id']?>" id="r-check-<?php echo $action['id']?>" <?php echo (@$this->actions_evaluators->{@$action['id']}->r == 1)?'checked':''; ?> /></td>
							<?php else:?>
								<td class="em-no no-action-r"></td>
							<?php endif?>
							<?php if($action['u'] == 1): ?>
								<td  id="u-check-<?php echo $action['id']?>" class="em-has-checkbox" ><input type="checkbox" class="em-modal-check u-check" name="u-check-<?php echo $action['id']?>" id="u-check-<?php echo $action['id']?>" <?php echo (@$this->actions_evaluators->{@$action['id']}->u == 1)?'checked':''; ?>/></td>
							<?php else:?>
								<td class="em-no no-action-u"></td>
							<?php endif?>
							<?php if($action['d'] == 1): ?>
								<td  id="d-check-<?php echo $action['id']?>" class="em-has-checkbox" ><input type="checkbox" class="em-modal-check d-check" name="d-check-<?php echo $action['id']?>" id="d-check-<?php echo $action['id']?>" <?php echo (@$this->actions_evaluators->{@$action['id']}->d == 1)?'checked':''; ?>/></td>
							<?php else:?>
