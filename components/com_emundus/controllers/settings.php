...
class EmundusControllersettings extends JControllerLegacy
{
    ...
    public function getemundusparamExtensions()
    {
        $db = JFactory::getDbo(); // Get the database object
        $query = $db->getQuery(true); // Create a new query object

        $query->select($db->quoteName('params')) // Select the manifest_cache column
        ->from($db->quoteName('#__extensions')) // From the jos_extensions table
        ->where($db->quoteName('name') . ' = ' . $db->quote('com_emundus')); // Where the name is com_emundus

        $db->setQuery($query); // Set the query

        $result = $db->loadResult(); // Load the result
    ...
    public function saveemundusparamExtensions()
    {
        ...
        $params = $jinput->json->get('data', '', 'RAW'); // Get JSON data from the request
            ...
            if (!empty($params)) {
                foreach ($params as $param) {
                    ...
                    if ($this->m_settings->updateEmundusParam($param->component, $param->param, $param->value)) {
                        ...
                        if ($param === 'list_limit') {
                            JFactory::getSession()->set('limit', $param->value);
                        }
                    } else {
                        $response['msg'] = JText::_('PARAM_NOT_UPDATED');
                    }
