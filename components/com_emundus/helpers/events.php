                ...
                if (isset($scholarship_document_id)) {
                    ...
                    if (empty($uploaded_document)) {
                        ...
                        if (!empty($scholarship_product)) {
                            ...
                            $checkout_url = 'index.php?option=com_hikashop&ctrl=product&task=cleancart&return_url=' . urlencode(base64_encode($return_url)) . '&reset=1';

                            JPluginHelper::importPlugin('emundus','custom_event_handler');
                            \Joomla\CMS\Factory::getApplication()->triggerEvent('callEventHandler', ['onBeforeEmundusRedirectToHikashopCheckout', ['url' => $checkout_url, 'fnum' => $user->fnum, 'user' => $user]]);
                ...
                if (count($fnumInfos) > 0) {
                    ...
                    if (!empty($checkout_cart_url)) {
                        ...
                        if ($accept_other_payments != 2 && empty($mApplication->getHikashopOrder($fnumInfos)) && $attachments >= 100 && $forms >= 100) {
                            ...
                            $checkout_url = 'index.php?option=com_hikashop&ctrl=product&task=cleancart&return_url=' . urlencode(base64_encode($checkout_url)) . '&reset=1';
