...
class EmundusModelCampaign extends JModelList {
    ...
    function getAssociatedCampaigns($filter, $sort, $recherche, $lim, $page,$program, $session) {
        ...
        $query
            ...
            ->where($this->_db->quoteName('sc.training') . ' IN (' . implode(',',$this->_db->quote($programs)) . ')');
            if(!empty($filterDate)) {
        ...
        try {
            ...
            return array('datas' => $campaigns,'count' => $campaigns_count);
        } catch (Exception $e) {
    ...
    public function getWorkflowById($id): stdClass
    {
        ...
        if (!empty($id)) {
            ...
            try {
                $workflow = $this->_db->loadObject();
            } catch (Exception $e) {
                JLog::add('[getCurrentCampaignWorkflow] Error getting campaign workflow by id in component/com_emundus/models/campaign: '.$e->getMessage(), JLog::ERROR, 'com_emundus.error');
            }
