    ...
    public function getProfileByCampaignWorkflow($campaign_id) {
        if(!empty($campaign_id)) {
            try {
                ...
                $this->_db->setQuery($query);
                $_defaultProfile = $this->_db->loadObject();

                /// check if default profile is in firstProfileList --> if yes, keep only firstProfileList, if no, keep both
                /// using in_array
                if(in_array($_defaultProfile->profile_id, $profile)) {
                    $profiles[] = $profile;
                } else {
                    $profiles[] = $profile;
                    $profiles[] = $_defaultProfile->profile_id;
                }
                ...
                $query->clear()
                    ...
                    ->where($this->_db->quoteName('#__emundus_setup_profiles.id') . 'IN (' . implode(',', $profiles[0]). ')');

                $this->_db->setQuery($query);
                $_profilesLabel = $this->_db->loadObjectList();

                foreach($_profilesLabel as $key=>$value) {
                    $profileLabels[] = $value->label;
                }

                return ['profile_id'=>$profiles, 'profile_label'=>$profileLabels];
            } catch(Exception $e) {
                return $e->getMessage();
            }
        } else {
