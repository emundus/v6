...
class EmundusModelUsers extends JModelList
{
	...
	public function _buildQuery()
	{
		...
		$query = 'SELECT u.id, e.lastname, e.firstname, u.email, u.username,  espr.label as profile, up.profile_value as newsletter, u.registerDate, u.lastvisitDate,  GROUP_CONCAT( DISTINCT esgr.label SEPARATOR "<br>") as groupe, cat.title as university, u.block as active
					FROM #__users AS u 
		...
		$query .= ' where (espr.id > 1 OR espr.id = 0 OR espr.id IS NULL) ';
		if($edit==1)
			$query.= ' u.id='.mysql_real_escape_string($uid);
		else
		{
			$and = true;
            if(isset($this->filts_details['profile_users']) && !empty($this->filts_details['profile_users'])){
                $query.= ' AND e.profile IN ('.implode(',', $this->filts_details['profile_users']).') ';
                $and = true;
            }
			if(isset($pid) && !empty($pid) && is_numeric($pid))
			{
				$query.= ' AND e.profile = "'.mysql_real_escape_string($pid).'"';
				$and = true;
			}
			if(isset($final_grade) && !empty($final_grade))
			{
