...
//die(str_replace('#_', 'jos', $query));
                            ...
                            if (isset($fnumInfo[$fnum])) {
                                ...
                                if (!$anonymize_data) {
                                    if($suffixe == null) {
                                        ...
                                        $filename = $_cFile->sanitize_filename($fnumInfo[$fnum]['applicant_name']) . '_' . $fnum . $attachInfo['lbl'] . '_' . $suffixe . '_' . ".docx";
                                    }
                                } else {
                                    if($suffixe == null) {
                                        $filename = $_cFile->sanitize_filename($fnum) . $attachInfo['lbl'] . '_' . ".docx";
                                    } else {
                                        $filename = $_cFile->sanitize_filename($fnum) . $attachInfo['lbl'] . '_' . $suffixe . '_' . ".docx";
                                    }
                                }
        ...
        if($showMode == 0) {
            ...
            foreach ($applicant_id as $key => $uid) {
                ...
                if($mergeMode == 0) {
                    ...
                    $is_Dir_Empty = $this->is_Dir_Empty(EMUNDUS_PATH_ABS . $uid . '--letters');                 /// return type :: BOOL

                    if($is_Dir_Empty) {
                        rmdir(EMUNDUS_PATH_ABS . $uid . '--letters');                                          /// if original folder is empty --> let remove it
                    } else {
                        $_zipName = $uid . '_' . date("Y-m-d") . '_' . '.zip';
                ...
                if($mergeMode == 1) {
                    ...
                    $is_Dir_Empty = $this->is_Dir_Empty(EMUNDUS_PATH_ABS . $uid . '--letters');
    ...
    private static function is_Dir_Empty($dirname): bool {
