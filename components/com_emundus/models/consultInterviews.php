...
class EmundusModelConsultInterviews extends JModelLegacy {
...
{ 
 ...
 $db = JFactory::getDBO();
	...
	function getCoordinatorForMail()
{ 

$session = JFactory::getSession();
$coordName = $session->get('user');

return $coordName->name;
}
    ...
    $db->quoteName('lbl') . ' = ' . $db->quote('cancel_interview_to_candidate')
		...
		$user = $this->getCoordinatorForMail();
		$startdate = $this->getStartDate($calId);
		$enddate = $this->getEndDate($calId);
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
	...
	function getNameCandidate($calId){
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
	...
	function getMailCandidate($calId){
		$idCand = $this->getNameCandidate($calId);

		$db = JFactory::getDBO();
		$query = $db->getQuery(true);

		$conditions = array(
			$db->quoteName('id') . ' = ' . $db->quote($idCand));
  ...
  return $result;
	...
	// Envoi mail
       ...
       $model = new EmundusModelEmails;
		...
		$mailCand = $this->getMailCandidate($calId);
		$idCand = $this->getNameCandidate($calId);
	...
	function sendMailCoordinateur($calId){
       ...
       $model = new EmundusModelEmails;
		...
		$mailCoord = $this->getMailCoordinator($calId);
