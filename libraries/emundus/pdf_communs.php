    ...
    if (class_exists('MYPDF') === false || !class_exists('MYPDF')) {
        ...
        class MYPDF extends TCPDF {
            ...
            public function Footer() {
                ...
                // footer
                $this->writeHTMLCell($w=0, $h=0, $x='', $y=275, $this->footer, $border=0, $ln=1, $fill=0, $reseth=true, $align='', $autopadding=true);
                //logo
    ...
    if (class_exists('MYPDF') === false || !class_exists('MYPDF')) {
        ...
        class MYPDF extends TCPDF {
            ...
            public function Header() {
                // Logo
                if (is_file($this->logo)) {
                    $this->Image($this->logo, 0, 0, 50, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
                }
                // Set font
                $this->SetFont('helvetica', 'B', 16);
                // Title
                $this->writeHTMLCell($w=192, $h=15, $x=10, $y=0, $this->header, $border=0, $ln=1, $fill=1, $reseth=true, $align='', $autopadding=true);
            }
            ...
            public function Footer() {
                ...
                $this->SetFont('helvetica', 'I', 8);
                // footer
                $this->writeHTMLCell($w=0, $h=0, $x='', $y=275, $this->footer.' Page '.$this->getAliasNumPage().'/'.$this->getAliasNbPages().'</p>', $border=0, $ln=1, $fill=0, $reseth=true, $align='', $autopadding=true);
                //logo
                if (is_file($this->logo_footer)) {
                    $this->Image($this->logo_footer, 150, 280, 40, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
                }
...
//get title
    ...
    $pdf->header = '<header style="padding:30px;">
    
        <p style="color:white; vertical-align: center;margin-right:100px;"><img src="'.$logo.'" style="width: 150px; height:26px;margin-right:30px;">'.$title.' </p>
    ...
    $htmldata .=
        '<style>
	.card  { border: 1px solid black; display:block; line-height:80%;}
    ...
    if (class_exists('MYPDF') === false || !class_exists('MYPDF')) {
        ...
        class MYPDF extends TCPDF {
            ...
            public function Footer() {
                ...
                $this->SetFont('helvetica', 'I', 8);
                // footer
                $this->writeHTMLCell($w=0, $h=0, $x='', $y=285, $this->footer.' Page '.$this->getAliasNumPage().'/'.$this->getAliasNbPages().'</p>', $border=0, $ln=1, $fill=0, $reseth=true, $align='', $autopadding=true);
                //logo
                if (is_file($this->logo_footer)) {
                    $this->Image($this->logo_footer, 150, 275, 40, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
                }
            }
        }
    }
